...
static void set_gpio_strenth(U32 Group, U32 BitNumber, U32 mA) 
{
	...
	switch( mA )
	{   
		case 2: drv1 = 0, drv0 = 0; break;
		case 4: drv1 = 0, drv0 = 1; break;
		case 6: drv1 = 1, drv0 = 0; break;
		case 8: drv1 = 1, drv0 = 1; break;
	}   

	drv1_value = NX_GPIO_GetDRV1(Group); 
	drv0_value = NX_GPIO_GetDRV0(Group);
	drv1_value = drv1_value | drv1 << BitNumber ;
	drv0_value = drv1_value | drv0 << BitNumber ;
...
static void bd_bus_init(void)
{
	...
	busnum = sizeof(pads)/sizeof(pads[0]);
	for (bit = 0; busnum > bit; bit++) {
		buspad   = PAD_GET_BUSPAD  (pads[bit]);
		strength = PAD_GET_STRENGTH(pads[bit]);
...
#define CFG_PIO_USB_HUB_RESET 	(PAD_GPIO_A + 20)
static int __init board_late_init(void)
{
    int hub = CFG_PIO_USB_HUB_RESET;
    uint time = 2000;

    printk("USB Hub Enable and Wait %dms...\n", time);
    msleep(time);

    /* Enable USB Hub */
//  nxp_soc_gpio_set_io_func  (hub, NX_GPIO_PADFUNC_GPIO);
//  nxp_soc_gpio_set_io_dir   (hub, 1);
    nxp_soc_gpio_set_out_value(hub, 1);
}
late_initcall(board_late_init);
