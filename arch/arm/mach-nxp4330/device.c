...
#if defined(CONFIG_SPI_SPIDEV) || defined(CONFIG_SPI_SPIDEV_MODULE) //FIXME: sesters, just for testing
...
#if 1	/* Experiment: disable DDR mode on eMMC */
	/* Transfers seems to be reliable at 50MHz,
	 * and the effective iospeed for 50MHz/noDDR
	 * is roughly the same as for 25MHz/DDR */
	.caps		= MMC_CAP_CMD23 | MMC_CAP_4_BIT_DATA 
...
#include "soc/setup-usb-phy.c"

void __init *nxp4330_set_platdata(void *pd, size_t pdsize, struct platform_device *pdev)
{
	void *npd;
...
void __init nxp4330_ehci_set_platdata(struct nxp4330_ehci_platdata *pd)
{
	...
	npd = nxp4330_set_platdata(pd, sizeof(struct nxp4330_ehci_platdata), &nxp4330_device_ehci);
...
static void vin_setup_io2(int module, bool on)
{
    printk("%s: module:%d  on:%d\n", __func__, module, on);
}
...
static int hi253_power(struct device *dev, int on)
{
    ...
    printk("%s: %d", __func__, on);
    // FIXME ...

    if (on) {
    	// clock output @ ~21MHz (166MHz / 8)
    	nxp_soc_pwm_set_frequency(0, 166500000 >> 3, 50);
    	nxp_soc_gpio_set_out_value(PAD_GPIO_D + 1, 1);
    	nxp_soc_gpio_set_io_dir(PAD_GPIO_D + 1, 1);
    	nxp_soc_gpio_set_io_func(PAD_GPIO_D + 1, NX_GPIO_PADFUNC_1);
    	// nxp_soc_gpio_set_io_pullup(PAD_GPIO_D + 1, 1);
    	// enable low
        nxp_soc_gpio_set_out_value(PAD_GPIO_E + 13, 0);
        nxp_soc_gpio_set_io_dir(PAD_GPIO_E + 13, 1);
        nxp_soc_gpio_set_io_func(PAD_GPIO_E + 13, NX_GPIO_PADFUNC_0);
        // reset pulse low
//        nxp_soc_gpio_set_out_value(PAD_GPIO_E + 7, 0);
        nxp_soc_gpio_set_io_dir(PAD_GPIO_E + 7, 1);
        nxp_soc_gpio_set_io_func(PAD_GPIO_E + 7, NX_GPIO_PADFUNC_0);
//        mdelay(1);
        nxp_soc_gpio_set_out_value(PAD_GPIO_E + 7, 1);
        // FIXME: R3K board mod to share ENABLE & RESET lines. and input external HSYNC, VSYNC lines
//        nxp_soc_gpio_set_out_value(PAD_GPIO_C + 15, 0); // CAM1 ENABLE
//        nxp_soc_gpio_set_out_value(PAD_GPIO_C + 16, 1); // CAM1 RESET
    }
    else {
       	// disable high
        // nxp_soc_gpio_set_out_value(PAD_GPIO_E + 13, 1);
    }
...
static struct soc_camera_link hi253_link[] = {
	{
		...
		.power			= &hi253_power,
		.i2c_adapter_id		= 0,		/* i2c bus number */
		.query_bus_param	= lf2k_hi253_query_bus_param,
		.priv			= &rear_hi253_cam_info,
	},
	{
		.bus_id			= 1,	/* matches VIP platform_device id */
...
#endif /* CONFIG_V4L2_LF2000 || CONFIG_V4L2_LF2000_MODULE */
...
/* SP 093013 - Just copy-pasting this one for now, need to port its hooks */
...
void __init nxp_board_devices_register(void)
{
	...
	#endif
	#ifdef CONFIG_MMC_NEXELL_CH2
	nxp_dwmci_platform_device_register(&_dwmci2_data, 2, 100*1000*1000);
	#endif
#endif
