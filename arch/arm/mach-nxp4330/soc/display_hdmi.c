...
#define DEFAULT_SAMPLE_RATE             CFG_AUDIO_SPDIF_TX_SAMPLE_RATE
#define DEFAULT_BITS_PER_SAMPLE         16
#define DEFAULT_AUDIO_CODEC             HDMI_AUDIO_PCM

#define NXP_HDMIPHY_PRESET_TABLE_SIZE   32
...
static const u8 hdmiphy_preset74_25[32] = {
    ...
    0xe8, 0xba, 0xd8, 0x45, 0xa0, 0xac, 0xb0, 0x34,
    0x80, 0x09, 0x84, 0x05, 0x22, 0x24, 0x0e, 0x54,
...
#if 0	// FIXME: sesters, in Nexell git server, but also in mach-nxp4330/include/mach/display.h
...
static int _hdmi_phy_enable(struct nxp_hdmi_context *me, int enable)
{
    ...
    if (enable) {
        ...
        case NXP_HDMI_PRESET_480P:
            table = hdmiphy_preset27_027;
            break;
...
{
	nxp_soc_disp_device_get_param(device, (void*)par, sizeof(*par));
...
static int _set_remote_sync(struct nxp_hdmi_context *me)
{
    ...
    vsync.interlace_scan = 0;

    ret = nxp_soc_disp_device_set_param(source_device, (void*)&param, sizeof(param));
    if (ret) {
        pr_err("%s: failed to display parameter....\n", __func__);
        return ret;
    }

#if (0) // FIXME: recursive enable
...
#else
    // Update source device vsync timings for HDMI device
    nxp_soc_disp_device_set_vsync(source_device, &vsync);
#endif
...
int nxp_soc_disp_hdmi_set_preset(int preset)
{
    ...
    if (preset != me->cur_preset) {
    	printk("%s: setting HDMI preset mode %d\n", __func__, preset);
    	if (me->streaming)
...
static int hdmi_probe(struct platform_device *pdev)
{
    ...
    if (_context->cur_preset == NXP_HDMI_PRESET_720P) {
        _context->vic = 4;
    } else if (_context->cur_preset == NXP_HDMI_PRESET_480P) {
        _context->vic = 3; // FIXME
    } else if (_context->cur_preset == NXP_HDMI_PRESET_1080P) {
        _context->vic = 16;
    }


	nxp_soc_disp_device_set_param(plat->display_in, &param, sizeof(param));
	nxp_soc_disp_setup_proc_device(DISP_DEVICE_HDMI, 0, &hdmi_ops, NULL);
