...
inline static u_short R8G8B8toR5G6B5(unsigned int RGB)
{
...
inline static unsigned int R5G6B5toR8G8B8(u_short RGB)
{
...
inline static u_char R8G8B8toR3G3B2(unsigned int RGB)
{
	u_char	R = (u_char)((RGB>>16) & 0xff);
	u_char	G = (u_char)((RGB>>8 ) & 0xff);
	u_char	B = (u_char)((RGB>>0 ) & 0xff);
...
inline static unsigned int R3G3B2toR8G8B8(u_char RGB)
{
	u_char R3  = (RGB >> 5) & 0x7;
	u_char G3  = (RGB >> 2) & 0x7;
	u_char B2  = (RGB >> 0) & 0x3;
...
#define	get_display_base(d)		(device_dev[d].base_addr)
#define	get_display_par(d)		(device_dev[d].dev_param)
#define	set_display_ops(d, o)	{ if (o) { device_dev[d].disp_ops  = o; o->dev = &device_dev[d]; } }
#define	set_display_base(d, b)	{ if (b) device_dev[d].base_addr = b; }
#define	set_display_par(d, p)	{ if (p) device_dev[d].dev_param = p; }
...
static int disp_multily_enable(struct disp_control_info *info, int enable)
{
	...
	if (enable) {
		...
		pmly->interlace_scan = psync->interlace_scan;
		pmly->mem_lock_len = 16;	/* fix mem lock size, par->mem_lock_size */

		xresol = pmly->x_resol;
		yresol = pmly->y_resol;
		interlace = pmly->interlace_scan;
...
inline static void disp_syncgen_irqenable(int module, int enable)
{
...
#if 0
#warning "Must revert this code after next chip revision !!!"
	NX_MLC_SetTopDirtyFlag(module);
#endif
...
static void disp_syncgen_initialize(void)
{
	...
	printk("Display Reset Status : %s\n", power?"On":"Off");
	/* reset */
	if (! power) {
		disp_topctl_reset();
		disp_syncgen_reset();
		printk("Display Reset Top/Syncgen ...\n");
	}
...
static int  disp_syncgen_prepare(struct disp_control_info *info)
{
	...
	int interlace = psync->interlace_scan;
...
{
	struct disp_control_info *info = get_device_to_info(pdev);
	int wait = info->wait_time * 10, pend = 0;
	int module = info->module;
	...
	if (!enable) {
		...
		if (CTRUE == NX_DPC_GetDPCEnable(module)) {
			...
			do {
				mdelay(1);
				pend = (int)NX_DPC_GetInterruptPendingAll(module);
			} while (wait-- > 0 && !pend);
		}
...
void nxp_soc_disp_device_set_vsync(int device, struct disp_vsync_info *psync)
{
	struct disp_process_ops *ops = NULL;
	RET_ASSERT(DEVICE_SIZE > device);
	DBGOUT("%s: %s\n", __func__, dev_to_str(device));
	...
	ops = get_display_ops(device);
	if (ops && ops->set_vsync)
		ops->set_vsync(ops->dev, psync);
}

int	nxp_soc_disp_device_get_param(int device, void *par, int parlen)
{
	struct disp_process_dev *pdev = get_display_ptr(device);
	RET_ASSERT_VAL(par && parlen > 0, -EINVAL);
	RET_ASSERT_VAL(DEVICE_SIZE > device, -EINVAL);
	DBGOUT("%s: %s, param = %p\n", __func__, dev_to_str(device), pdev->dev_param);

	if (! pdev->dev_param) {
		printk(KERN_ERR "%s: Fail, %s parameter not registed ...\n",
			__func__, dev_to_str(device));
		return -EINVAL;
	}

	memcpy(par, pdev->dev_param, parlen);
	return 0;
}
...
int	nxp_soc_disp_device_set_param(int device, void *par, int parlen)
{
	...
	RET_ASSERT_VAL(par && parlen > 0, -EINVAL);
	RET_ASSERT_VAL(DEVICE_SIZE > device, -EINVAL);
	DBGOUT("%s: %s, param = %p (len:%d)\n",
		__func__, dev_to_str(device), pdev->dev_param, parlen);
	...
	memcpy(pdev->dev_param, par, parlen);
	return 0;
}
						...
						struct disp_process_ops *ops, void *param)
{
	...
	set_display_base(device, base);
	set_display_par (device, param);
...
EXPORT_SYMBOL(nxp_soc_disp_device_set_vsync);
...
EXPORT_SYMBOL(nxp_soc_disp_setup_proc_device);
...
{
	struct nxp_syncgen_plat_data *plat = platdev->dev.platform_data;
	...
	if (plat) {
		if (plat->vsync)
			memcpy(&pdev->vsync, plat->vsync, sizeof(struct disp_vsync_info));
		if (plat->par)
			memcpy(pdev->dev_param, plat->par, sizeof(struct disp_syncgen_param));
	}
...
static int display_soc_probe(struct platform_device *platdev)
{
	struct disp_control_info *info;
	struct disp_process_dev *pdev = NULL;
	struct disp_multily_dev *pmly = NULL;
	int module = platdev->id;
	...
	/* set control info */
	ret = display_soc_setup(module, pdev, platdev);
	if (0 > ret){
		kfree(info);
		return ret;
	}
