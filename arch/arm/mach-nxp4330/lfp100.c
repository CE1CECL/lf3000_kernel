...
extern int HP_Connect_Status_Update(int status2,unsigned char *DigVolReg);
...
static void lfp100_monitor_task(struct work_struct *work)
{
	...
	unsigned char volume_reg, val;
	
	if (HP_Connect_Status_Update(reg, &val) == 0)
	{
			/* digital volume update */
		volume_reg = lfp100_read_reg_raw(LFP100_VOLUME);

		volume_reg &= 0xC0;

		volume_reg |= val;

		lfp100_write_reg_raw(LFP100_VOLUME, volume_reg);
	}
