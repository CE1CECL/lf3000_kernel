...
static void init_santiago(void)
{
	printk(KERN_WARNING "%s: Santiago GPIO mapping\n", __func__);
	...
	/* NC             - pin A11 */
	/* NC             - pin A12 */
#ifndef CONFIG_PLAT_NXP4330_SANTIAGO	/* Not connected in Santiago */
	gpio_map[I2S_SEL_BT] 		= LF3000_GPIO_PORT_A | 13;
#endif
	/* NC             - pin A14 */
#ifndef CONFIG_PLAT_NXP4330_SANTIAGO	/* Not connected in Santiago */
    ...
    gpio_map[PMIC_INT]    = LF3000_GPIO_PORT_C | 4;
#else
    gpio_map[CHG_INT]     = LF3000_GPIO_PORT_C | 4;
#endif
	/* BT_HOST_CTS    - pin  C5 */
	/* BT_HOST_RTS    - pin  C6 */
	gpio_map[WIFI_RESET]		= LF3000_GPIO_PORT_C | 7;
	...
	gpio_map[ACCEL_INT]		= LF3000_GPIO_PORT_C | 8;
#ifndef CONFIG_PLAT_NXP4330_SANTIAGO	/* BT_RESET_L not in Santiago */
	gpio_map[BT_RESET_L] 		= LF3000_GPIO_PORT_C | 9;
#endif
	gpio_map[WIFI_HOST_WAKE] 	= LF3000_GPIO_PORT_C | 10;
	/* NC	          - pin C11 */
#ifndef CONFIG_PLAT_NXP4330_SANTIAGO	/* BT_LINK not in Santiago */
	gpio_map[BT_LINK] 		= LF3000_GPIO_PORT_C | 12;
#endif
#ifdef CONFIG_PLAT_NXP4330_SANTIAGO	/* LED_ENA is on C13 in Santiago */
	gpio_map[LED_ENA]		= LF3000_GPIO_PORT_C | 13; //LCD_LED_ENA
#endif
	gpio_map[REAR_CAM_RESET_L]	= LF3000_GPIO_PORT_C | 14; //VID0_RESET_L
	gpio_map[FRONT_CAM_ENABLE_L]	= LF3000_GPIO_PORT_C | 15; //VID1_ENA_L
...
#ifndef CONFIG_PLAT_NXP4330_SANTIAGO	/* AUDIO_MUTE and AUDIO_RST_L not in Santiago */
	gpio_map[AUDIO_MUTE]		= LF3000_GPIO_PORT_E | 16;
	gpio_map[AUDIO_RST_L]		= LF3000_GPIO_PORT_E | 17;
#endif
	/* NC             - pin E18 */
	...
	gpio_map[BUTTON_HOME]	= LF3000_GPIO_PORT_E | 20;
	gpio_map[DPAD_DOWN]		= LF3000_GPIO_PORT_E | 21;
#ifndef CONFIG_PLAT_NXP4330_SANTIAGO	/* LED_ENA is on C13 in Santiago */
	gpio_map[LED_ENA]		= LF3000_GPIO_PORT_E | 22; //LCD_LED_ENA
#endif
	gpio_map[DPAD_RIGHT]	= LF3000_GPIO_PORT_E | 23;
...
static void init_cabo(void)
{
	...
	gpio_map[DPAD_LEFT]		= LF3000_GPIO_PORT_E | 9;
	gpio_map[HEADPHONE_JACK] = LF3000_GPIO_PORT_E | 10;
#ifndef CONFIG_REGULATOR_NXE2000	/* nxe2000.h #defines CHG_INT differently */
    gpio_map[CHG_INT]        = LF3000_GPIO_PORT_E | 11;
#else
    gpio_map[PMIC_INT]       = LF3000_GPIO_PORT_E | 11;
#endif
        /* N/C           - pin E12 */	
        /* N/C           - pin E13 */
	/* SCK_R         - pin E14 */		
...
void lf3000_gpio_init_map(void)
{
	...
	switch(system_rev) {
		...
		default:
			...
			init_xanadu();
#elif defined(CONFIG_PLAT_NXP4330_SANTIAGO)
			printk(KERN_WARNING "defaulting to SANTIAGO GPIO mapping\n");
			init_santiago();
#else
