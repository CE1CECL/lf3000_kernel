...
static ssize_t sysfs_show_lfp100_locale(struct device *dev,
                                     struct device_attribute *attr,
                                     char *buf)
{
	return sprintf(buf, "%s\n", lfp100_get_locale());
...
static ssize_t sysfs_store_lfp100_locale(struct device *dev,
                                      struct device_attribute *attr,
			              const char *buf,
			              size_t count)
{
	char str[count];
	sscanf(buf, "%s", str);
	lfp100_set_locale(str);
	return count;
}

static DEVICE_ATTR(tc94b26_locale, S_IRUGO | S_IWUGO, sysfs_show_lfp100_locale, sysfs_store_lfp100_locale);
