...
/* GPIO Input values */
static u32 suspend_alfn[5][2] = {
    {0x00000000, 0x00000000},   // GPIO A
    {0x66800000, 0x55555566},   // GPIO B
    {0x55555555, 0x00555555},   // GPIO C
    {0x00000000, 0x00000000},   // GPIO D
    {0x00000000, 0x55540000}    // GPIO E
};
static void suspend_gpio(suspend_state_t stat)
{
	...
	if (SUSPEND_SUSPEND == stat) {
		for (i = 0; size > i; i++, gpio++, base += 0x1000) {
			...
			writel((-1UL), (base+0x4C));
			writel((-1UL), (base+0x54));
			writel((-1UL), (base+0x5C));
			writel((-1UL), (base+0x64));
			...
			writel(suspend_alfn[i][0], (base+0x20));
			writel(suspend_alfn[i][1], (base+0x24));
		}
	} else {
		for (i = 0; size > i; i++, gpio++, base += 0x1000) {
