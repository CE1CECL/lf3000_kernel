...
static void cpu_base_init(void)
{
	...
	NX_TIMER_Initialize();
	for (i = 0; NX_TIMER_GetNumberOfModule() > i; i++) {
		NX_TIMER_SetBaseAddress(i, (U32)IO_ADDRESS(NX_TIMER_GetPhysicalAddress(i)));
		NX_TIMER_OpenModule(i);
	}

	NX_PWM_Initialize();
	for (i = 0; NX_PWM_GetNumberOfModule() > i; i++) {
		NX_PWM_SetBaseAddress(i, (U32)IO_ADDRESS(NX_PWM_GetPhysicalAddress(i)));
		NX_PWM_OpenModule(i);
	}
}
