...
#if 0	/* 28oct13  Don't enable CONFIG_MTD_NAND_NEXELL_PARTS_MAP, because */
	/*		cfg_nand_map.h doesn't exist */
...
{
#if 1	/* 29oct13 */
	/* Minh indicated that the clock is running at 1.003 MHz */
	float ticks_per_usec = 1000000.0 / 1003000.0;
...
static ssize_t set_ecc_mode( int oc, const char *buf, size_t count)
{
...
#if 0	/* 31oct13 */
	case 40:
	case 51:	/* stand-in for 24 bits / 512 bytes */
	case 60:
		break;
#endif	/* 31oct13 */
	default:
	...
	if (new_mode == 0) {
		...
		retval = nand_hw_ecc_init(mtd, new_mode);
...
/*-----------------------------------------------------------------*/
#if 0	/* 6nov13 */
static int cart_inserted = 0;

int is_cart_inserted(void) {
	return cart_inserted;
}
EXPORT_SYMBOL(is_cart_inserted);

static ssize_t set_cart_inserted( struct device *dev,
			     struct device_attribute *attr, 
			     const char *buf, size_t count)
{
	unsigned int value;
	...
	cart_inserted = (value) ? 1 : 0;

	return count;
}

static ssize_t show_cart_inserted(struct device *dev, 
			     struct device_attribute *attr, char *buf)
{
	ssize_t len;

	len = sprintf(buf, "cart is %sinserted; gpio %d, !gpio %d\n", 
			cart_inserted ? "" : "NOT ",
			gpio_get_value(CARTRIDGE_DETECT),
			!gpio_get_value(CARTRIDGE_DETECT));
	return len;
}

static DEVICE_ATTR(cart_inserted, 
		   S_IRUSR|S_IRGRP|S_IROTH|S_IWUSR|S_IWGRP|S_IWOTH, 
		   show_cart_inserted, set_cart_inserted);
...
#if 1	// FIXME: TODO: Do we want/need something like this, or can we
...
#if 0	/* 6nov13  Just for testing */
	&dev_attr_cart_inserted.attr,
#endif	/* 6nov13 */
...
static void onboard_nand_select(struct mtd_info *mtd, int chipnr)
{
	...
	if (-1 == chipnr) {
		...
		if (chip->ecc.mode == NAND_ECC_HW) {
			...
			NX_MCUS_SetECCMode(pnd->hw_ecc_level);
		}
		NX_MCUS_SetInterruptEnable( 0, CTRUE);
		NX_GPIO_SetPadFunction(1, 14, NX_GPIO_PADFUNC_1);
	}
}
...
static void cart_nand_select(struct mtd_info *mtd, int chipnr)
{
	...
	if (-1 == chipnr) {
		...
		NX_MCUS_SetInterruptEnable( 0, CTRUE);
		NX_GPIO_SetPadFunction(1, 14, NX_GPIO_PADFUNC_0);
	}
}
// Till here
		            	 ...
		            	 struct mtd_oob_ops *ops)
{
	...
	if (!ops->datbuf) {
/* saw lots of these while checking Hynix NAND for bad blocks
printk(KERN_INFO "lf2000_nand_read_oob() calls nand_do_read_oob()\n");
 */
		ret = nand_do_read_oob(mtd, from, ops);
	}
	else {
printk(KERN_INFO "lf2000_nand_read_oob() calls nand_lf2000_nand_do_read_ops()\n");
		ret = lf2000_nand_do_read_ops(mtd, from, ops);
	}
 out:
	nand_release_device(mtd);
...
#if 1	/* 28oct13 */
	printk(KERN_INFO "%s: system_rev is 0x%x\n", __func__, system_rev);
#endif	/* 28oct13 */
	...
	switch(system_rev) {
		...
		default: 
			printk(KERN_ERR "%s GPIO mapping does not support " \
				"Board Revision = 0x%X\n",
				__func__, system_rev);
#if 1	/* 28oct13; keep this until system_rev is init'd correctly */
			chip->select_chip = onboard_nand_select;
#endif	/* 28oct13 */
	}
...
#if 1	/* 4nov13 */
printk(KERN_INFO "%s: GPIOB14: AltFunc-%d, %sput, pullup %s\n",
	__func__, 
	NX_GPIO_GetPadFunction(1, 14),
	NX_GPIO_GetOutputEnable(1,14) ? "out" : "in",
	NX_GPIO_GetPullUpEnable(1,14) ? "on" : "off");

	NX_GPIO_SetPadFunction(1, 14, NX_GPIO_PADFUNC_1);
	NX_GPIO_SetOutputEnable(1,14, 0);
	NX_GPIO_SetPullUpEnable(1,14, 1);

printk(KERN_INFO "%s: Later: GPIOB14: AltFunc-%d, %sput, pullup %s\n",
	__func__, 
	NX_GPIO_GetPadFunction(1, 14),
	NX_GPIO_GetOutputEnable(1,14) ? "out" : "in",
	NX_GPIO_GetPullUpEnable(1,14) ? "on" : "off");
...
#endif	// 14jul11
	{
		printk(KERN_INFO "%s: exit after nand_scan\n", __func__);
		ret = -ENXIO;
#if 1	/* 28oct13 */
...
printk(KERN_INFO "onboard chip options: 0x%08x\n", chip->options);
...
#if 0	/* 28oct13  Disabled for now since I think it won't be executed */
	...
	if (0 >= parts_nr) {
		cmd_part  = 0;
#if 0	/* 28oct13  Disabled in order to get error-free compilation */
...
#else
		data->parts = partition_info_cart;
		parts_nr  = ARRAY_SIZE(partition_info_cart);
		def_mtd_parts(&data->mtd, partition_info_cart);
#endif	/* 28oct13 */
	}
