...
static void debug_sync(unsigned long priv)
{
    ...
    /* printk("HSYNC FUNC %d, VSYNC FUNC %d\n", */
    /*         nxp_soc_gpio_get_io_func(PAD_GPIO_E + 5), nxp_soc_gpio_get_io_func(PAD_GPIO_E + 6)); */
...
static int _hw_set_output_format(struct nxp_vin_clipper *me)
{
    ...
    if (me->platdata->is_mipi) {
        ...
        pr_debug("%s: mipi set clipper format to 0x%x\n", __func__, nx_format);
    } else {
    ...
    NX_VIP_SetClipperFormat(module, nx_format, me->hflip, me->vflip, me->rotate);
...
static int _hw_set_addr(struct nxp_vin_clipper *me, struct nxp_video_buffer *buf)
{
    ...
    if (me->platdata->is_mipi) {
        struct v4l2_rect *c = &me->crop;
        /* printk("%s: set mipi vip format(0x%x), width %d, height %d, buf y 0x%x, buf cb 0x%x, buf cr 0x%x, stride 0 %d, stride 1 %d\n", */
        /*         __func__, */
        /*         NX_VIP_FORMAT_422, */
        /*         c->width - c->left, */
        /*         c->height - c->top, */
...
static void late_power_down(unsigned long priv)
{
    struct nxp_vin_clipper *me = (struct nxp_vin_clipper *)priv;
...
static int _configure(struct nxp_vin_clipper *me, int enable)
{
    ...
    if (enable) {
        if (me->platdata->late_power_down)
            del_timer(&me->timer_power_down);
        _hw_set_sensor_param(me);
        _hw_set_input_size(me);
        _hw_set_output_format(me);
        _hw_set_crop(me);

#if 0 /* FIXME removing redundant power on (sounds like bad idea to keep enabling) */
        ...
        if (me->platdata->late_power_down)
            mod_timer(&me->timer_power_down, jiffies + msecs_to_jiffies(LATE_POWERDOWN_TIMEOUT_MS));

        if (NXP_ATOMIC_READ(&me->state) & NXP_VIN_STATE_STOPPING)
            NXP_ATOMIC_CLEAR_MASK(NXP_VIN_STATE_STOPPING, &me->state);

#if 0 /* FIXME leaving out until further discoveries are made as to when/where to power on/off */
        /* turn camera off */
        if (me->platdata->power_enable)
                    me->platdata->power_enable(false);
#endif
...
#if 0       // FIXME -- defer from irq handler
...
static int nxp_vin_clipper_s_power(struct v4l2_subdev *sd, int on)
{
    ...
    if (state & NXP_VIN_STATE_RUNNING_CLIPPER) {
        // FIXME -- stream irq left on
        pr_err("%s error: running state(0x%x)\n", __func__, state);
...
static int nxp_vin_clipper_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
    ...
    case V4L2_CID_SATURATION:
    	phyaddr = (unsigned long)ctrl->value;
    	if (!width)
    		width = 400;
    	if (!height)
    		height = 300;
        printk("decimator %s: source(%dx%d), target(%dx%d)\n", __func__,
            me->crop.width, me->crop.height, width, height);
        NX_VIP_SetDecimation(module, me->crop.width, me->crop.height, width, height);
        NX_VIP_SetDecimatorFormat(module, NX_VIP_FORMAT_420, false, false, false);
        NX_VIP_SetDecimatorAddr(module, NX_VIP_FORMAT_420, width, height,
            phyaddr, phyaddr+2048, phyaddr+4096*height/2, 4096, 4096);
        break;
    case V4L2_CID_HFLIP:
    	me->hflip = ctrl->value;
    	break;
...
static int nxp_vin_clipper_s_stream(struct v4l2_subdev *sd, int enable)
{
    ...
    if (enable) {
        ...
        if (is_host_video) {
            if (NXP_ATOMIC_READ(&me->state) & NXP_VIN_STATE_RUNNING_CLIPPER) {
                printk("clipper video stopping...\n");
                NXP_ATOMIC_SET_MASK(NXP_VIN_STATE_STOPPING, &me->state);
                if (!wait_for_completion_interruptible_timeout(&me->stop_done, 2*HZ)) {
                    printk("wait timeout for stopping\n");
                    parent->stop(parent, me);
                }
                // FIXME -- deferred from clipper_irq_handler()
                _unregister_irq_handler(me);
                _clear_buf(me);
                NXP_ATOMIC_CLEAR_MASK(NXP_VIN_STATE_STOPPING, &me->state);
                NXP_ATOMIC_CLEAR_MASK(NXP_VIN_STATE_RUNNING_CLIPPER, &me->state);
        ...
        const struct media_pad *remote, u32 flags)
{
    ...
    case NXP_VIN_PAD_SINK | MEDIA_ENT_T_V4L2_SUBDEV:
        ...
        pr_debug("%s: connect clipper to sensor\n", __func__);
        _handle_input_connection(me, true);
        break;

    case NXP_VIN_PAD_SOURCE_MEM | MEDIA_ENT_T_DEVNODE:
        pr_debug("%s: link video to clipper %d, enable %d\n", __func__, local->index, flags & MEDIA_LNK_FL_ENABLED);
        if (!(flags & MEDIA_LNK_FL_ENABLED)) {
    ...
    case NXP_VIN_PAD_SOURCE_DECIMATOR | MEDIA_ENT_T_V4L2_SUBDEV:
        pr_debug("%s: link decimator to clipper %d, enable %d\n", __func__, local->index, flags & MEDIA_LNK_FL_ENABLED);
        if (!(flags & MEDIA_LNK_FL_ENABLED)) {
...
#endif
        setup_timer(&me->timer_power_down, late_power_down, (long)me);
