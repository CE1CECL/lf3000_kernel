        ...
        struct disp_vsync_info *vsync, struct disp_syncgen_param *par)
{
    ...
    nxp_soc_disp_device_get_param(device, (void *)par, sizeof(*par)); // FIXME
...
static int _set_remote_sync(struct nxp_resc *me)
{
    ...
    if (!strncmp(remote_source->name, "NXP MLC", 7)) {
        ...
        ret = nxp_soc_disp_device_set_param(source_device, (void *)&param, sizeof(param)); // FIXME
        if (ret) {
