...
#include "lf2000/hynix_yac_dat.h"
#include "lf2000/hi253_default.h"
#include "lf2000/hi253_capture.h"

#define ADDR_S5K5CAGX 	0x20 //0x3c
...
#define MAX_WIDTH           1600 //2048
#define MAX_HEIGHT          1200 //1536
...
static int s5k5cagx_write_regs(struct i2c_client *client, unsigned char (*regvals)[3], int size)
{
    unsigned char tmp[4];
    int i, err = 0;
    ...
    for (i = 0; i < size ; i++) {
...
#else
        {
           	if (page != REG_MAP[regvals[i][0]]) {
           		...
           		err = s5k5cagx_i2c_write(client, tmp , 2);
...
static int s5k5cagx_set_frame_size(struct v4l2_subdev *sd, int mode, int width, int height)
{
    ...
    if (width > 800 || height > 600)
        return s5k5cagx_write_regs(client, HI253_Capture_UXGA, ARRAY_SIZE(HI253_Capture_UXGA));
    else if (width > 400 || height > 300)
        return s5k5cagx_write_regs(client, HI253_Capture_SVGA, ARRAY_SIZE(HI253_Capture_SVGA));
    return s5k5cagx_write_regs(client, HI253_Capture_QSVGA, ARRAY_SIZE(HI253_Capture_QSVGA));
#endif
}
...
static int s5k5cagx_initialize_ctrls(struct s5k5cagx_state *state)
{
    ...
    state->exposure = v4l2_ctrl_new_std(&state->handler, &s5k5cagx_ctrl_ops,
...
{
#if 1
    if (read_device_id_for_s5k5cagx(client) == 0x92) // HI253
        return 0;
    return -1;
#else
    u32 id;
...
static int s5k5cagx_init(struct v4l2_subdev *sd, u32 val)
{
    ...
    if (!state->inited) {
        ...
        err = s5k5cagx_write_regs(client, HI253_Default, ARRAY_SIZE(HI253_Default));
        if (err < 0) {
...
static int read_device_id_for_s5k5cagx(struct i2c_client *client)
{
    ...
    id = i2c_read_reg(client,0x0, 0x04);
