...
static const struct regval_list sp2518_svga_init_regs[] =
{
    ...
    {0x26, 0x25},
    {0x27, 0x6d},
    {0x2c, 0x23},//31 //0x33 //zouyu 20130315
    {0x2d, 0x75},
    {0x2e, 0x38},//18
    // psw0523 here... plck
    ...
    {0x32, 0x00},
    // psw0523 add
    {0x36, 0x02}, // bit1: ccir656 output enable
    // end psw0523
    {0xfd, 0x01},
    {0x2c, 0x00},
    {0x2d, 0x00},
    ...
    {0x9d, 0x38},//;40
    /*
    //SI50_SP2518 UXGA 24MEclk 2倍频 1分频 50Hz 9.0208-9fps AE_Parameters_20121218161146.txt
    //ae setting
    {0xfd,0x00},
    {0x03,0x02},
    {0x04,0xbe},
    ...
    */
    ...
    {0xc4, 0xa0},//raw_rb_fac_indoor
    {0x43, 0xa0},//raw_rb_fac_dummy
    {0xad, 0x40},//w_rb_fac_low
...
static const struct sp2518_win_size *sp2518_select_win(u32 width, u32 height)
{
    ...
    printk(KERN_ERR "%s: unsupported width, height\n", __func__);
    return NULL;
}
...
static int sp2518_set_white_balance_temperature(struct v4l2_subdev *sd, struct v4l2_ctrl *ctrl)
{
    ...
    switch(white_balance_temperature) {
        ...
        case V4L2_WHITE_BALANCE_TUNGSTEN:
            ret = sp2518_write_array(sp2518_wb_tungsten_regs);
            break;
        default:
...
static int sp2518_s_stream(struct v4l2_subdev *sd, int enable)
{
    ...
    if (enable) {
        ...
        sp2518_set_exposure_param(sd);
    } else {
        if (priv->prev_capt_mode == PREVIEW_MODE) {
            ret = sp2518_save_exposure_param(sd);
        }
    }
...
static void sp2518_priv_init(struct sp2518_priv * priv)
{
    ...
    priv->exposure_auto->cur.val = 1;
    priv->auto_white_balance->cur.val = 1;
