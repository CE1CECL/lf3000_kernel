		...
		struct nxe2000_regulator_platform_data *nxe2000_pdata)
{
...
#ifdef CONFIG_PLAT_NXP4330_SANTIAGO	/* maybe not needed, since this is NXE2000_ID_DC2 */
	if (0 == strcmp(consumer_supplies->supply, "V1.0_CORE")) {
	...
	if (nxe2000_pdata->init_uV > -1) {
		ret = __nxe2000_set_voltage(parent, ri,	nxe2000_pdata->init_uV, nxe2000_pdata->init_uV, NULL, 0);
		if (ret < 0) {
			dev_err(ri->dev, "Not able to initialize voltage %d for rail %d err %d\n", nxe2000_pdata->init_uV, ri->desc.id, ret);
			return ret;
		}
	...
	if (nxe2000_pdata->sleep_slots == -1) {
		ret = __nxe2000_set_voltage(parent, ri,	nxe2000_pdata->init_uV, nxe2000_pdata->init_uV, NULL, 1);
		if (ret < 0) {
