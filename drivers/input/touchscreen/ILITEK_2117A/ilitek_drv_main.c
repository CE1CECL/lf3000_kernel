...
#elif defined(ILITEK_EARLYSUSPEN)
...
static u8 _gIsFirstTouchKeyPressed = 0;
...
//        gpio_set_value(MS_TS_MSG_IC_GPIO_RST, 1); 
       ...
       nxp_soc_gpio_set_out_value(MS_TS_MSG_IC_GPIO_RST, 0);
        mdelay(100);
        //gpio_set_value(MS_TS_MSG_IC_GPIO_RST, 1);
	nxp_soc_gpio_set_out_value(MS_TS_MSG_IC_GPIO_RST, 1);
        mdelay(25); 
    }
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
#ifdef CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
...
void DrvFingerTouchPressed(s32 nX, s32 nY, s32 nPressure, s32 nId)
{
	...
	if(nId == 0)
	{
	//	DBG("1 Point, ID=%02X, X=%04d, Y=%04d,touch_key_hold_press=%d\n",nId, x,y,touch_key_hold_press);
		input_report_abs(g_InputDevice, ABS_X, nY);
		input_report_abs(g_InputDevice, ABS_Y, TOUCH_SCREEN_X_MAX - nX);
		input_report_key(g_InputDevice, BTN_TOUCH, 1);
		input_report_abs(g_InputDevice, ABS_PRESSURE, 40);
	}
    ...
    input_report_abs(g_InputDevice, ABS_MT_POSITION_X, nY);
    input_report_abs(g_InputDevice, ABS_MT_POSITION_Y, TOUCH_SCREEN_X_MAX - nX);
    input_report_abs(g_InputDevice, ABS_MT_PRESSURE, 40);
...
void DrvFingerTouchReleased(s32 nX, s32 nY, s32 nId)
{
	...
	if(nId == 0)
	{
		input_report_abs(g_InputDevice, ABS_PRESSURE, 0);
...
s32 DrvIliTekReadTouchPanelInfo(IliTekTpInfo_t *pTpInfo)
{
    ...
    if (g_ChipType == CHIP_TYPE_ILI2120)
    {
        ...
        {
            szWriteBuf[0] = ILITEK_TP_CMD_READ_DATA_CONTROL;
            szWriteBuf[1] = ILITEK_TP_CMD_GET_KEY_INFORMATION;
            rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szWriteBuf[0], 2);
            if (rc < 0)
            {
                DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_READ_DATA_CONTROL & ILITEK_TP_CMD_GET_KEY_INFORMATION : IicWriteData() failed, rc = %d\n", rc);
                return -1;
            }    
    ...
    if (0 == _DrvSelfParsePacket(pPacket, nReportPacketLength, &tInfo))
    {
        ...
        if ((tInfo.nFingerNum) == 0)   //touch end
        {
            ...
            else
            {
				...
				input_report_abs(g_InputDevice, ABS_MT_PRESSURE, 0);
                input_report_key(g_InputDevice, BTN_TOUCH, 0);
...
s32 DrvInputDeviceInitialize(struct i2c_client *pClient)
{
    ...
    g_InputDevice->name = "touchscreen interface";//pClient->name;
	...
	set_bit(EV_SYN, g_InputDevice->evbit);
	set_bit(EV_KEY, g_InputDevice->evbit);
	set_bit(ABS_X, g_InputDevice->absbit);
	set_bit(ABS_Y, g_InputDevice->absbit);
	set_bit(ABS_PRESSURE, g_InputDevice->absbit);
	set_bit(BTN_TOUCH, g_InputDevice->keybit);
	input_set_abs_params(g_InputDevice, ABS_PRESSURE, 0, 100, 0, 0);
	//input_set_abs_params(input, ABS_X, 0, LCD_SIZE_X, 0, 0);
	//input_set_abs_params(input, ABS_Y, 0, LCD_SIZE_Y, 0, 0);
    input_set_abs_params(g_InputDevice, ABS_Y, TOUCH_SCREEN_X_MIN, TOUCH_SCREEN_X_MAX, 0, 0);
    input_set_abs_params(g_InputDevice, ABS_X, TOUCH_SCREEN_Y_MIN, TOUCH_SCREEN_Y_MAX, 0, 0);
#ifdef CONFIG_TP_HAVE_KEY
    // Method 1.
    { 
...
//#endif //VIRTUAL_KEY_PAD
    ...
    input_set_abs_params(g_InputDevice, ABS_MT_TOUCH_MAJOR, 0, 255, 0, 0);
    input_set_abs_params(g_InputDevice, ABS_MT_WIDTH_MAJOR, 0, 255, 0, 0);
    input_set_abs_params(g_InputDevice, ABS_MT_POSITION_Y, TOUCH_SCREEN_X_MIN, TOUCH_SCREEN_X_MAX, 0, 0);
    input_set_abs_params(g_InputDevice, ABS_MT_POSITION_X, TOUCH_SCREEN_Y_MIN, TOUCH_SCREEN_Y_MAX, 0, 0);
    input_set_abs_params(g_InputDevice, ABS_MT_PRESSURE, 0, 100, 0, 0);

#ifdef CONFIG_ENABLE_TYPE_B_PROTOCOL
    if (g_ChipType == CHIP_TYPE_MSG22XX)
    {
        set_bit(BTN_TOOL_FINGER, g_InputDevice->keybit);
        input_mt_init_slots(g_InputDevice, SELF_MAX_TOUCH_NUM); // for MSG22xx
    }
    else if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2118A || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121)
    {
        set_bit(BTN_TOOL_FINGER, g_InputDevice->keybit);
        input_mt_init_slots(g_InputDevice, MUTUAL_MAX_TOUCH_NUM);  // for MSG28xx/MSG58xxA/ILI21xx 
    }
#endif //CONFIG_ENABLE_TYPE_B_PROTOCOL
...
#endif //CONFIG_ENABLE_TOUCH_PIN_CONTROL
	DBG(&g_I2cClient->dev, "*** MS_TS_MSG_IC_GPIO_RST %d***\n", MS_TS_MSG_IC_GPIO_RST); 
...
//	MS_TS_MSG_IC_GPIO_INT = pClient->irq;
	DBG(&g_I2cClient->dev, "*** MS_TS_MSG_IC_GPIO_INT %d***pClient:%d\n", MS_TS_MSG_IC_GPIO_INT,pClient->irq); 
...
s32 DrvTouchDeviceRegisterFingerTouchInterruptHandler(void)
{
    ...
    _gIrq = g_I2cClient->irq; //gpio_to_irq(MS_TS_MSG_IC_GPIO_INT);
    DBG(&g_I2cClient->dev, "*** %s() *** _gIrq = %d\n", __func__, _gIrq); 
    ...
    if (pPacket[0] == 0x5A)
    {
        ...
        {    
            ...
            if (g_TouchKObj != NULL)
            {
                ...
                for (i = 0; i < nLength; i ++)
                {
                    sprintf(szValue, "%02x", g_DemoModePacket[i] & 0xFF);
