...
u8 finger_num=0;
int valid_i2c_register = 0;
static char DBG_FLAG;
static int line_width = 40;
static const char *goodix_ts_name = "CTPM_ts";
...
struct goodix_ts_data *gt_data;


// LF PARAMS

// SZ: params are at /sys/devices/platform/nxp-i2c.2/i2c-2/2-005d/

static short report_pressure = 40; // 40 = default pressure to report

static ssize_t show_report_pressure(struct device* cd,
                     struct device_attribute *attr, char* buf)
{
    ssize_t ret = 0;
    sprintf(buf, "%hd\n", report_pressure);
    ret = strlen(buf) + 1;
    printk("report_pressure: %hd, ret: %d\n", report_pressure, ret);
    return ret;
}
static ssize_t store_report_pressure(struct device* cd, struct device_attribute *attr, const char* buf, size_t len)
{
    printk("len: %d, report_pressure: %s\n", len, buf);
    sscanf(buf, "%hd", &report_pressure);
    return len;
}
...
static DEVICE_ATTR(report_pressure, S_IRUGO | S_IWUSR, show_report_pressure, store_report_pressure);

/********************* Ming add for file_operation *****************/
/*
description
...
*/
static int 
gt9xx_file_open(struct inode *inode, struct file *filp)
{
	//GTP_DEBUG("%s\n",__func__);
	return 0; 
}
	...
	struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	...
	if(valid_i2c_register == 0){
		printk(GT9XX_ERROR_LEVEL"%s, i2c device driver doesn't be registered\n", __func__);
		return -1;
	}

	// check the buffer size whether it exceeds the local buffer size or not
	if(count > 128){
		printk(GT9XX_ERROR_LEVEL "%s, buffer exceed 128 bytes\n", __func__);
		return -1;
	}
	...
	if(ret < 0){
		printk(GT9XX_ERROR_LEVEL "%s, copy data from user space, failed", __func__);
		return -1;
	}

	// parsing command
	if(strcmp(buffer, "dbg") == 0){
		GTP_DEBUG_ON=!GTP_DEBUG_ON;
		printk("%s, %s message(%X).\n",__func__,DBG_FLAG?"Enabled":"Disabled",DBG_FLAG);
	}else if(strcmp(buffer, "info") == 0){
		;//gt9xx_i2c_read_tp_info();
	...
	/************** Ming add end *****************/
		...
		//	UPDATE_FILE_PATH = filename;
			autoUpdate = 0;
    			ret = gup_init_update_proc(gt_data);
    			if (ret < 0)
    			{
        			GTP_ERROR("Create update thread error.");
   			}
		//}
	}else if(strstr(buffer,"upgradecfg") == buffer){
#if 0
		//spilt command
		char *const splitForCom = " ";
		char *tokenForCom, *curForCom = buffer;
...
struct file_operations gt9xx_fops = {
	.read = gt9xx_file_read,
	.write = gt9xx_file_write,
	.open = gt9xx_file_open,
	.release = gt9xx_file_close,
};
/************************************************************/
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
    ...
    if(id == 0)
    {
    	GTP_DEBUG("Single Touch report!");
    	input_report_abs(ts->input_dev, ABS_X, x);
    	input_report_abs(ts->input_dev, ABS_Y, y);
	if(finger_num == 1)
	{
   		input_report_abs(ts->input_dev, ABS_PRESSURE, report_pressure?report_pressure:w);
   		input_report_key(ts->input_dev, BTN_TOUCH, 1);
	}
    }
//SZ1 #if GT9XX_MULTITOUCH
#if GTP_ICS_SLOT_REPORT
    ...
    input_report_abs(ts->input_dev, ABS_MT_PRESSURE, report_pressure?report_pressure:w);
#else
    GTP_DEBUG("Multi Touch report!");
    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, x);
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, y);
    input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, w);
    input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, w);
    input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, id);
//SZ1    input_mt_sync(ts->input_dev);
#endif
//SZ1 #else
       
//SZ1 #endif

GTP_DEBUG("ID:%d, X:%d, Y:%d, W:%d, W1:%d", id, x, y, w, report_pressure?(int)report_pressure:w);
}
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if (touch_num)
    {
        for (i = 0; i < touch_num; i++)
        {
            ...
            gtp_touch_down(ts, id, input_x, input_y, input_w);
	    GTP_DEBUG("gtp_touch_down x= %d y=%d\n",input_x, input_y);
        }
    }
    else if (pre_touch)
    {
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    //SZSZ GTP_GPIO_AS_INPUT(GTP_RST_PORT);    // end select I2C slave addr
...
#endif
    //SZSZ GTP_GPIO_AS_INPUT(GTP_RST_PORT);
    gtp_reset_guitar(ts->client, 20);
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    ts->input_dev->evbit[0] = BIT_MASK(EV_KEY) | BIT_MASK(EV_ABS); // org BIT_MASK(EV_SYN) | BIT_MASK(EV_KEY) | BIT_MASK(EV_ABS) ;
#if GTP_ICS_SLOT_REPORT
...
//SZ1    #else
    ...
    input_set_abs_params(ts->input_dev, ABS_PRESSURE, 0, 100, 0, 0);
    /*** Ming add for single touch end ***/
//SZ1    #endif
    sprintf(phys, "input/ts");
    ts->input_dev->name = "touchscreen interface";
...
/**/
	...
	ret = alloc_chrdev_region(&dev.devno, 0, 1, GTP_FILE_DRIVER_NAME);
	if(ret){
		printk(GT9XX_ERROR_LEVEL "%s, can't allocate chrdev\n", __func__);
	return ret;
	}
	printk(GT9XX_DEBUG_LEVEL "%s, register chrdev(%d, %d)\n", __func__, MAJOR(dev.devno), MINOR(dev.devno));
	// initialize character device driver
	cdev_init(&dev.cdev, &gt9xx_fops);
	dev.cdev.owner = THIS_MODULE;
	ret = cdev_add(&dev.cdev, dev.devno, 1);
	if(ret < 0){
		printk(GT9XX_ERROR_LEVEL "%s, add character device error, ret %d\n", __func__, ret);
	return ret;
	}
	dev.class = class_create(THIS_MODULE, GTP_FILE_DRIVER_NAME);
	if(IS_ERR(dev.class)){
		printk(GT9XX_ERROR_LEVEL "%s, create class, error\n", __func__);
	return ret;
	}
	device_create(dev.class, NULL, dev.devno, NULL, "gt9xx_ctrl");

	//LF STRUCT
	device_create_file(&(client->dev), &(dev_attr_report_pressure));
