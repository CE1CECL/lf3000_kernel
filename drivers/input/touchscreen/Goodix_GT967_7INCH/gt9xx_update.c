...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
    ...
    GTP_DEBUG1("Sensor_ID: %d", sensor_id);
    ...
    if (ret == SUCCESS)
    {
        GTP_DEBUG1("CFG_GROUP%d Config Version: %d, IC Config Version: %d", sensor_id+1, 
    ...
    GTP_DEBUG1("X_MAX = %d, Y_MAX = %d, TRIGGER = 0x%02x",
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    GTP_DEBUG1("IC Hardware info:%02x%02x%02x%02x", update_msg.ic_fw_msg.hw_info[0], update_msg.ic_fw_msg.hw_info[1],
    ...
    GTP_DEBUG1("IC force update flag:0x%x", update_msg.force_update);
    ...
    GTP_DEBUG1("IC Product id:%s", update_msg.ic_fw_msg.pid);
    ...
    if(update_msg.ic_fw_msg.pid[0] != 0)
    {
        ...
        {
            GTP_DEBUG1("IC Mapping Product id:%s", update_msg.ic_fw_msg.pid);
            memcpy(update_msg.ic_fw_msg.pid, "9110P", 5);
        }
    }
    
    update_msg.ic_fw_msg.vid = buf[GTP_ADDR_LENGTH+4] + (buf[GTP_ADDR_LENGTH+5]<<8);
    GTP_DEBUG1("IC version id:%04x", update_msg.ic_fw_msg.vid);
...
s32 gup_enter_update_mode(struct i2c_client *client)
{
    ...
    while(retry++ < 200)
    {
        ...
        if(ret <= 0)
        {
            GTP_DEBUG1("Hold ss51 & dsp I2C error,retry:%d", retry);
            continue;
        }
        ...
        {
            GTP_DEBUG1("Hold ss51 & dsp confirm SUCCESS");
            break;
        }
        GTP_DEBUG1("Hold ss51 & dsp confirm 0x4180 failed,value:%d", rd_buf[GTP_ADDR_LENGTH]);
    }
    if(retry >= 200)
    {
...
{
    //SZSZ need to keep it as output till reset is complete 
    // GTP_GPIO_AS_INT(GTP_INT_PORT);
    
    GTP_DEBUG1("[leave_update_mode]reset chip.");
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    GTP_DEBUG1("FILE HARDWARE INFO:%02x%02x%02x%02x", fw_head->hw_info[0], fw_head->hw_info[1], fw_head->hw_info[2], fw_head->hw_info[3]);
    GTP_DEBUG1("FILE PID:%s", fw_head->pid);
    GTP_DEBUG1("FILE VID:%04x", fw_head->vid);

    GTP_DEBUG1("IC HARDWARE INFO:%02x%02x%02x%02x", update_msg.ic_fw_msg.hw_info[0], update_msg.ic_fw_msg.hw_info[1],
                                                   update_msg.ic_fw_msg.hw_info[2], update_msg.ic_fw_msg.hw_info[3]);
    GTP_DEBUG1("IC PID:%s", update_msg.ic_fw_msg.pid);
    GTP_DEBUG1("IC VID:%04x", update_msg.ic_fw_msg.vid);

    //First two conditions
    if ( !memcmp(fw_head->hw_info, update_msg.ic_fw_msg.hw_info, sizeof(update_msg.ic_fw_msg.hw_info)))
    {
        GTP_DEBUG1("Get the same hardware info.");
            ...
            (!memcmp(fw_head->pid, "91XX", 4)))
        {
            ...
            {
                GTP_DEBUG1("Force none same pid update mode.");
            }
            else
            {
                GTP_DEBUG1("Get the same pid.");
            }
            //The third condition
	    if(autoUpdate)
	    {
...
static s8 gup_update_config(struct i2c_client *client)
{
    ...
    pid[5] = '\0';
    GTP_DEBUG1("update cfg get pid:%s", &pid[GTP_ADDR_LENGTH]);
    ...
    GTP_DEBUG1("[update_cfg]config file len:%d", file_len);
    GTP_DEBUG1("[update_cfg]need config len:%d",chip_cfg_len);
    ...
    GTP_DEBUG1("[update_cfg]Read config from file.");
    ...
    GTP_DEBUG1("[update_cfg]Delete illgal charactor.");
    ...
    GTP_DEBUG1("[update_cfg]Ascii to hex.");
...
static u8 gup_check_fs_mounted(char *path_name)
{
    ...
    if (err)
    {
        GTP_DEBUG1("/data/ NOT Mounted: %d", err);
        return FAIL;
    }
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    if (path)
    {
        GTP_DEBUG1("Update File path:%s, %d", path, strlen(path));
    ...
    else
    {
        ...
        {
            GTP_DEBUG1("Waiting for /data mounted [%d]", i);

            if (gup_check_fs_mounted("/data") == SUCCESS)
            {
                GTP_DEBUG1("/data Mounted!");
                break;
            }
            msleep(3000);
        }
        ...
        {
            GTP_DEBUG1("%s is unavailable", CONFIG_FILE_PATH_1);
        }
        else
        {
        ...
        //{
            ...
            GTP_DEBUG1("%3d:Searching %s %s file...", i, (got_file_flag&0x0F)?"":"bin", (got_file_flag&0xF0)?"":"cfg");
            //msleep(3000);
        //}
        searching_file = 0;
        kfree(search_update_path);
        ...
        if(got_file_flag&0xF0)
        {
            GTP_DEBUG1("Got the update config file.");
            ret = gup_update_config(client);
            if(ret <= 0)
            {
                GTP_ERROR("Update config failed.");
        ...
        if(got_file_flag&0x0F)
        {
            GTP_DEBUG1("Got the update firmware file.");
        }
        else
        {
    ...
    for(i=0; i<FW_SECTION_LENGTH*4+FW_DSP_ISP_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH; i+=2)
    {
        ...
        //GTP_DEBUG1("BUF[0]:%x", buf[0]);
        temp = (buf[0]<<8) + buf[1];
        fw_checksum += temp;
    }
    
    GTP_DEBUG1("firmware checksum:%x", fw_checksum&0xFFFF);
...
static u8 gup_burn_proc(struct i2c_client *client, u8 *burn_buf, u16 start_addr, u16 total_length)
{
    ...
    GTP_DEBUG1("Begin burn %dk data to addr 0x%x", (total_length/1024), start_addr);
    while(burn_length < total_length)
    {
        GTP_DEBUG1("B/T:%04d/%04d", burn_length, total_length);
        ...
        for(retry = 0; retry < MAX_FRAME_CHECK_TIME; retry++)
        {
            ...
            if(memcmp(&wr_buf[GTP_ADDR_LENGTH], &rd_buf[GTP_ADDR_LENGTH], frame_length))
            {
                ...
                GTP_DEBUG1("write array:");
                GTP_DEBUG_ARRAY(&wr_buf[GTP_ADDR_LENGTH], frame_length);
                GTP_DEBUG1("read array:");
                GTP_DEBUG_ARRAY(&rd_buf[GTP_ADDR_LENGTH], frame_length);
                continue;
            }
            else
            {
                //GTP_DEBUG1("Check frame data success.");
                break;
            }
        }
...
static u8 gup_recall_check(struct i2c_client *client, u8* chk_src, u16 start_rd_addr, u16 chk_length)
{
    ...
    while(recall_length < chk_length)
    {
        ...
        if(memcmp(&rd_buf[GTP_ADDR_LENGTH], &chk_src[recall_length], frame_length))
        {
            ...
            GTP_DEBUG1("chk_src array:");
            GTP_DEBUG_ARRAY(&chk_src[recall_length], frame_length);
            GTP_DEBUG1("recall array:");
            GTP_DEBUG_ARRAY(&rd_buf[GTP_ADDR_LENGTH], frame_length);
            return FAIL;
        }
    ...
    GTP_DEBUG1("Recall check %dk firmware success.", (chk_length/1024));
...
static u8 gup_burn_fw_section(struct i2c_client *client, u8 *fw_section, u16 start_addr, u8 bank_cmd )
{
    ...
    GTP_DEBUG1("[burn_fw_section]Wait for the burn is complete......");
    do{
        ...
        msleep(10);
        //GTP_DEBUG1("[burn_fw_section]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]);
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
    ...
    GTP_DEBUG1("[burn_dsp_isp]Begin burn dsp isp---->>");
    
    //step1:alloc memory
    GTP_DEBUG1("[burn_dsp_isp]step1:alloc memory");
    while(retry++ < 5)
    {
    ...
    GTP_DEBUG1("[burn_dsp_isp]step2:load dsp isp file data");
    ...
    GTP_DEBUG1("[burn_dsp_isp]step3:disable wdt,clear cache enable");
    ...
    //step4:hold ss51 & dsp
    GTP_DEBUG1("[burn_dsp_isp]step4:hold ss51 & dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
    if(ret <= 0)
    {
    ...
    GTP_DEBUG1("[burn_dsp_isp]step5:set boot from sram");
    ...
    GTP_DEBUG1("[burn_dsp_isp]step6:software reboot");
    ...
    //step7:select bank2
    GTP_DEBUG1("[burn_dsp_isp]step7:select bank2");
    ...
    GTP_DEBUG1("[burn_dsp_isp]step8:enable accessing code");
    ret = gup_set_ic_msg(client, _bRW_MISCTL__MEM_CD_EN, 0x01);
    if(ret <= 0)
    {
    ...
    GTP_DEBUG1("[burn_dsp_isp]step9:burn 4k dsp_isp");
    ...
    //step10:set scramble
    GTP_DEBUG1("[burn_dsp_isp]step10:set scramble");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_OPT_B0_, 0x00);
    if(ret <= 0)
    {
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
    ...
    GTP_DEBUG1("[burn_fw_ss51]Begin burn ss51 firmware---->>");
    
    //step1:alloc memory
    GTP_DEBUG1("[burn_fw_ss51]step1:alloc memory");
    while(retry++ < 5)
    {
    ...
    GTP_DEBUG1("[burn_fw_ss51]step2:load ss51 firmware section 1 file data");
    ...
    GTP_DEBUG1("[burn_fw_ss51]step3:clear control flag");
    ...
    GTP_DEBUG1("[burn_fw_ss51]step4:burn ss51 firmware section 1");
    ...
    GTP_DEBUG1("[burn_fw_ss51]step5:load ss51 firmware section 2 file data");
    ...
    GTP_DEBUG1("[burn_fw_ss51]step6:burn ss51 firmware section 2");
    ...
    GTP_DEBUG1("[burn_fw_ss51]step7:load ss51 firmware section 3 file data");
    ...
    GTP_DEBUG1("[burn_fw_ss51]step8:burn ss51 firmware section 3");
    ...
    GTP_DEBUG1("[burn_fw_ss51]step9:load ss51 firmware section 4 file data");
    ...
    GTP_DEBUG1("[burn_fw_ss51]step10:burn ss51 firmware section 4");
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
    ...
    GTP_DEBUG1("[burn_fw_dsp]Begin burn dsp firmware---->>");
    //step1:alloc memory
    GTP_DEBUG1("[burn_fw_dsp]step1:alloc memory");
    while(retry++ < 5)
    {
    ...
    GTP_DEBUG1("[burn_fw_dsp]step2:load firmware dsp");
    ...
    //step3:select bank3
    GTP_DEBUG1("[burn_fw_dsp]step3:select bank3");
    ...
    //step4:hold ss51 & dsp
    GTP_DEBUG1("[burn_fw_dsp]step4:hold ss51 & dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
    if(ret <= 0)
    {
    ...
    //step5:set scramble
    GTP_DEBUG1("[burn_fw_dsp]step5:set scramble");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_OPT_B0_, 0x00);
    if(ret <= 0)
    {
    ...
    GTP_DEBUG1("[burn_fw_dsp]step6:release ss51 & dsp");
    ...
    GTP_DEBUG1("[burn_fw_dsp]step7:burn 4k dsp firmware");
    ...
    GTP_DEBUG1("[burn_fw_dsp]step8:send burn cmd to move data to flash from sram");
    ...
    GTP_DEBUG1("[burn_fw_dsp]Wait for the burn is complete......");
    do{
        ret = gup_get_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, rd_buf, 1);
        if(ret <= 0)
        {
        ...
        msleep(10);
        //GTP_DEBUG1("[burn_fw_dsp]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]);
    }while(rd_buf[GTP_ADDR_LENGTH]);
    
    //step9:recall check 4k dsp firmware
    GTP_DEBUG1("[burn_fw_dsp]step9:recall check 4k dsp firmware");
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
    ...
    GTP_DEBUG1("[burn_fw_boot]Begin burn bootloader firmware---->>");
    
    //step1:Alloc memory
    GTP_DEBUG1("[burn_fw_boot]step1:Alloc memory");
    while(retry++ < 5)
    {
    ...
    GTP_DEBUG1("[burn_fw_boot]step2:load firmware bootloader");
    ...
    //step3:hold ss51 & dsp
    GTP_DEBUG1("[burn_fw_boot]step3:hold ss51 & dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
    if(ret <= 0)
    {
    ...
    //step4:set scramble
    GTP_DEBUG1("[burn_fw_boot]step4:set scramble");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_OPT_B0_, 0x00);
    if(ret <= 0)
    {
    ...
    GTP_DEBUG1("[burn_fw_boot]step5:release ss51 & dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x04);                 //20121211
    if(ret <= 0)
    {
    ...
    //step6:select bank3
    GTP_DEBUG1("[burn_fw_boot]step6:select bank3");
    ret = gup_set_ic_msg(client, _bRW_MISCTL__SRAM_BANK, 0x03);
    if(ret <= 0)
    {
    ...
    GTP_DEBUG1("[burn_fw_boot]step7:burn 2k bootloader firmware");
    ...
    //step7:send burn cmd to move data to flash from sram
    GTP_DEBUG1("[burn_fw_boot]step7:send burn cmd to move data to flash from sram");
    ...
    GTP_DEBUG1("[burn_fw_boot]Wait for the burn is complete......");
    do{
        ret = gup_get_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, rd_buf, 1);
        if(ret <= 0)
        {
        ...
        msleep(10);
        //GTP_DEBUG1("[burn_fw_boot]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]);
    }while(rd_buf[GTP_ADDR_LENGTH]);
    
    //step8:recall check 2k bootloader firmware
    GTP_DEBUG1("[burn_fw_boot]step8:recall check 2k bootloader firmware");
    ...
    GTP_DEBUG1("[burn_fw_boot]step9:enable download DSP code ");
    ...
    GTP_DEBUG1("[burn_fw_boot]step10:release ss51 & hold dsp");
...
s32 gup_update_proc(void *dir)
{
    ...
    GTP_DEBUG1("[update_proc]Begin update ......");
    ...
    GTP_DEBUG1("[update_proc]leave update mode.");
    gup_leave_update_mode();
    
    msleep(100);
//    GTP_DEBUG1("[update_proc]send config.");
