...
#define DRIVER_VERSION "20180504_v1"
...
/* In Lowcost board, GPIO output 3.3V but ADC read only 1.8V
 * In this case, a resistor and a capacitor is add to lower the Voltage
 * And ADC from X2 and Y2 is disconnected from the touchscreen to ensure correct voltage in X1 and Y1
 */
...
#define MAX_TNT_UP      50
#define MIN_TNT_DOWN    520
...
#define REMASK_ADC_DBG_MODE	(1 << REBIT_ADC_DBG_MODE)
	...
	int x2y2_adc_disable;   // HACK for Lowcost Board which X2/Y2 are removed from ADC
	...
	int	adc_max_tnt_up;   // tnt readings must be <= this for touch
	int	adc_min_tnt_down;     // either tnt reading > this ==> Up
	int	adc_max_delta_tnt;  // for Down, both tnt readings must be 
				    	// less than or equal to adc_max_tnt_up
	...
	unsigned gpio_ctl;      // GPIO for Control
	unsigned gpiofn_x1;		// GPIO function for X1
	unsigned gpiofn_x2;		// GPIO function for X2
	unsigned gpiofn_y1;		// GPIO function for Y1
	unsigned gpiofn_y2;		// GPIO function for Y2
	unsigned gpiofn_ctl;    // GPIO function for control
} touch_dev = {
	NULL, 		// i_dev
	NULL, 		// iraw_dev
	NULL,		// ts_task
	...
	0,		    // adc_tnt1
	0,		    // adc_tnt2
	MAX_TNT_UP,	//adc_max_tnt_up
	MIN_TNT_DOWN,   // adc_min_tnt_down
	...
	0xFFFF,			// GPIO CTL, mark as invalid
	GPIO_GPIOFN,    // GPIO X1, default function
	GPIO_GPIOFN,    // GPIO X2, default function
	GPIO_GPIOFN,    // GPIO Y1, default function
	GPIO_GPIOFN,    // GPIO Y2, default function
	GPIO_GPIOFN,    // GPIO CTL, default function
};
...
static ssize_t show_max_tnt_up(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct touch *lf2000_touch_dev = (struct touch *)dev->p->driver_data;
	return sprintf(buf, "%d\n", lf2000_touch_dev->adc_max_tnt_up);
}

static ssize_t set_max_tnt_up(struct device *dev,
			...
			const char *buf, size_t count)
{
	...
	lf2000_touch_dev->adc_max_tnt_up = temp;
...
static DEVICE_ATTR(max_tnt_up, \
	S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH, \
	show_max_tnt_up, set_max_tnt_up);


static ssize_t show_min_tnt_down(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct touch *lf2000_touch_dev = (struct touch *)dev->p->driver_data;
	return sprintf(buf, "%d\n", lf2000_touch_dev->adc_min_tnt_down);
}

static ssize_t set_min_tnt_down(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	int temp;
	...
	lf2000_touch_dev->adc_min_tnt_down = temp;
...
static DEVICE_ATTR(min_tnt_down, \
	S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH, \
	show_min_tnt_down, set_min_tnt_down);
...
static struct attribute *touchscreen_attributes[] = {
	...
	&dev_attr_max_tnt_up.attr,
	&dev_attr_min_tnt_down.attr,
...
{
    if (t_dev->gpio_ctl)
		lf2k_gpio_configure_pin(t_dev->gpio_ctl, t_dev->gpiofn_ctl, 1, 0, 1);

	switch (t_dev->tnt_mode)
	{
	case 0:
	case 2:
		// Set X1 and X2 as inputs without pullup
		lf2k_gpio_configure_pin(t_dev->gpio_x1, t_dev->gpiofn_x1, 0, 0, 0);
		lf2k_gpio_configure_pin(t_dev->gpio_x2, t_dev->gpiofn_x2, 0, 0, 0);
		// Set Y1 and Y2 as outputs high
		lf2k_gpio_configure_pin(t_dev->gpio_y1, t_dev->gpiofn_y1, 1, 0, 1);
		lf2k_gpio_configure_pin(t_dev->gpio_y2, t_dev->gpiofn_y2, 1, 0, 1);
		break;
	case 1:
	case 3:
		// Input on Y without pullup, Drive X low
		lf2k_gpio_configure_pin(t_dev->gpio_y1, t_dev->gpiofn_y1, 0, 0, 0);
		lf2k_gpio_configure_pin(t_dev->gpio_y2, t_dev->gpiofn_y2, 0, 0, 0);
		// Set X1 and X2 as outputs high
		lf2k_gpio_configure_pin(t_dev->gpio_x1, t_dev->gpiofn_x1, 1, 0, 1);
		lf2k_gpio_configure_pin(t_dev->gpio_x2, t_dev->gpiofn_x2, 1, 0, 1);
		break;
	}
}
...
static void set_read_p12(void)
{
	...
	lf2k_gpio_configure_pin(t_dev->gpio_y1, t_dev->gpiofn_y1, 0, 0, 0);
	lf2k_gpio_configure_pin(t_dev->gpio_x1, t_dev->gpiofn_x1, 1, 0, 1);
	lf2k_gpio_configure_pin(t_dev->gpio_y2, t_dev->gpiofn_y2, 1, 0, 0);
}
...
static void set_read_x(void)
{
    ...
    if (t_dev->gpio_ctl)
		lf2k_gpio_configure_pin(t_dev->gpio_ctl, t_dev->gpiofn_ctl, 1, 0, 0);
	// lf2k_gpio_configure_pin(log'l gpio, FUNCTION, OUT=1, PULLUP=1, VALUE)
	lf2k_gpio_configure_pin(t_dev->gpio_x1, t_dev->gpiofn_x1, 0, PU, 0);
	lf2k_gpio_configure_pin(t_dev->gpio_x2, t_dev->gpiofn_x2, 0, PU, 0);
	lf2k_gpio_configure_pin(t_dev->gpio_y1, t_dev->gpiofn_y1, 1, 0, 1);
	lf2k_gpio_configure_pin(t_dev->gpio_y2, t_dev->gpiofn_y2, 1, 0, 0);
}
...
static void set_read_y(void)
{
    ...
    if (t_dev->gpio_ctl)    
		lf2k_gpio_configure_pin(t_dev->gpio_ctl, t_dev->gpiofn_ctl, 1, 0, 0);
	// lf2k_gpio_configure_pin(log'l gpio, FUNCTION, OUT=1, PULLUP=1, VALUE)
	lf2k_gpio_configure_pin(t_dev->gpio_y1, t_dev->gpiofn_y1, 0, PU, 0);
	lf2k_gpio_configure_pin(t_dev->gpio_y2, t_dev->gpiofn_y2, 0, PU, 0);
	lf2k_gpio_configure_pin(t_dev->gpio_x1, t_dev->gpiofn_x1, 1, 0, 1);
	lf2k_gpio_configure_pin(t_dev->gpio_x2, t_dev->gpiofn_x2, 1, 0, 0);
}
 ...
 *
 * If both tnt readings are >= t_dev->adc_min_tnt_down and are close to each
 * other (differ by no more than t_dev->adc_max_delta_tnt), returns RXY_DOWN.  
 * If both tnt readings are >= t_dev->adc_min_tnt_down and differ by more
 * than t_dev->adc_max_delta_tnt, returns RXY_IN_BETWEEN.  
 *
 * Also returns RXY_IN_BETWEEN if both tnt readings are >= t_dev->adc_max_tnt_up
 * and at least one of them is > t_dev->adc_min_tnt_up.
 */
...
//#define USE_FANCY_TNT
	...
	first_reading_tnt();
	if (t_dev->adc_tnt1 < t_dev->adc_max_tnt_up) {
		t_dev->adc_tnt2 = 0; // indicate unread (to prevent lots of output)
	...
	if (t_dev->averaging > 1)
	{
		...
		if (t_dev->x2y2_adc_disable)
			t_dev->adc_x2 = t_dev->adc_x;
	}
	...
	if (t_dev->adc_tnt1 + t_dev->adc_tnt2 < 2*t_dev->adc_max_tnt_up)
        return RXY_UP;   // UP unless both tnt readings are high enough
	...
	fancy_tnt = t_dev->adc_max_tnt_up +
		(t_dev->tnt_plane[0] * x +
		 t_dev->tnt_plane[1] * y +
		 t_dev->tnt_plane[2] + 32768)/65536;
	if (t_dev->adc_tnt1 + t_dev->adc_tnt2 <= 2*fancy_tnt)
#else
	if (t_dev->adc_tnt1 + t_dev->adc_tnt2 >= 2*t_dev->adc_min_tnt_down)
#endif
    {
        // both are high enough for Down; check for stability
        ...
        // else both are > t_dev->adc_max_tnt_up and at least one is greater 
        // than t_dev->adc_min_tnt_down.
...
static void get_touch(struct work_struct *work)
{
	...
	if (t_dev->ts_state == TSTATE_DOWN) {
		...
		t_dev->screen_y = (t_dev->a[4] * t_dev->touch_x +
				   ...
				   t_dev->a[3]) / t_dev->a[6];
		if (t_dev->x2y2_adc_disable) {
			t_dev->adc_pressure = 500;  //HACK: Alway return ADC Pressure 500 if no avaliable
			t_dev->touch_pressure = 50; //HACK: Alway return Pressure 50% if no avaliable
		}
		else if (t_dev->adc_p2 == 0) {
	...
	// /sys/devices/platform/lf2000-touchscreen/report_events
...
#define TS_Y2		3
#define TS_CTL      4
#define NUM_TS_GPIOS	5

static const struct tag_ts_gpio ts_gpios_lowcost[NUM_TS_GPIOS] = {
	{TOUCHSCREEN_X1,        GPIO_GPIOFN,    "TS_X1"},
	{TOUCHSCREEN_X2,        GPIO_ALT1,      "TS_X2"},
	{TOUCHSCREEN_Y1,        GPIO_GPIOFN,    "TS_Y1"},
	{TOUCHSCREEN_Y2,        GPIO_ALT1,      "TS_Y2"},
	{TOUCHSCREEN_CTL,       GPIO_ALT1,      "TS_CTL"},
};
...
static int lf2000_ts2_probe(struct platform_device *pdev)
{
	...
	t_dev->gpio_ctl = ts_gpio_list[TS_CTL].gpio;

    /* setup gpio fn mapping */
	t_dev->gpiofn_x1 = ts_gpio_list[TS_X1].gpiofn;
	t_dev->gpiofn_x2 = ts_gpio_list[TS_X2].gpiofn;
	t_dev->gpiofn_y1 = ts_gpio_list[TS_Y1].gpiofn;
	t_dev->gpiofn_y2 = ts_gpio_list[TS_Y2].gpiofn;
	t_dev->gpiofn_ctl = ts_gpio_list[TS_CTL].gpiofn;
