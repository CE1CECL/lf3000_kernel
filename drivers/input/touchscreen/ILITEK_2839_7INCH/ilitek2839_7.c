...
static int ilitek_upgrade_firmware(void)
{
	...
	unsigned long ap_startaddr, df_startaddr, ap_endaddr, df_endaddr, ap_checksum = 0, df_checksum = 0, temp = 0;
	...
	df_len = ( CTPM_FW[26] << 16 ) + ( CTPM_FW[26] << 8 ) + CTPM_FW[28];
	...
	if(buffer[0] == 0x0B && buffer[1] == 0x00) {
		ice_flag = 1;
		chip_id_H = 0x22;
		chip_id_L = 0x10;
		ic_model = (chip_id_H << 8) + chip_id_L;
	...
	if(buffer[0] == 0x10 && buffer[1] == 0x22) {
		ice_flag = 1;
		chip_id_H = 0x22;
		chip_id_L = 0x10;
		ic_model = (chip_id_H << 8) + chip_id_L;
	...
	if(buffer[0] == 0x11 && buffer[1] == 0x22) {
		ice_flag = 1;
		chip_id_H = 0x22;
		chip_id_L = 0x11;
		ic_model = (chip_id_H << 8) + chip_id_L;
	...
	if(buffer[0] == 0x17 && buffer[1] == 0x21) {
		ice_flag = 1;
		chip_id_H = 0x21;
		chip_id_L = 0x17;
		ic_model = (chip_id_H << 8) + chip_id_L;
	...
	if ((buffer[0] == 0x03 || buffer[0] == 0x09 || buffer[1] == 0x23))
	{
		handle_df = true;
