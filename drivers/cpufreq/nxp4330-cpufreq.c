...
static void nxp4330_cpufreq_thread(struct work_struct *work)
{
	...
	pr_debug("%s: over freq %ldkhz %4ldms retention, change %u -> %u khz \n",
		__func__, dvfs->max_cpufreq, dvfs->max_retention, freqs.old, freqs.new);
...
static inline void nxp4330_cpufreq_setup(void)
{
	...
	dvfs->run_monitor = 0;
	INIT_DELAYED_WORK_DEFERRABLE(&dvfs->work, nxp4330_cpufreq_thread);
}
				...
				unsigned int relation)
{
	...
	pr_debug("%s: %u -> %u khz ", __func__, freqs.old, freqs.new);
	...
	if (ktime_to_ms(dvfs->rest_ktime) && freqs.new > dvfs->rest_cpufreq) {
		...
		if (dvfs->rest_retention > ts) {
			pr_debug("rest retention %4ld:%4ldms\n", dvfs->rest_retention, ts);
			return -EINVAL;	/* retry */
		}
	...
	if (dvfs->max_cpufreq && dvfs->run_monitor && freqs.new < dvfs->max_cpufreq ) {
		dvfs->run_monitor = 0;
		cancel_delayed_work_sync(&dvfs->work);
		pr_debug("stop monitor ");
	}

	if (dvfs->max_cpufreq && !dvfs->run_monitor && freqs.new >= dvfs->max_cpufreq) {
		dvfs->run_monitor = 1;
		schedule_delayed_work_on(dvfs->cpu, &dvfs->work,
