...
struct lf2000_hwmon {
	...
	enum	lf2000_power_chip chip;		/* support LF1000 / LFP100 / TC7734  /NXE2000 */
...
#else
	if(hwmon_dev->chip = NXE2000)
		return nxe2000_get_current_voltage();   //for santiago
...
#ifdef CONFIG_PLAT_NXP4330_SANTIAGO
	power_source = nxe2000_power_status();
#else
				...
				struct device_attribute *attr, char *buf)
{
	...
	power_source = lf2000_get_power_source(priv->on_battery);
#endif
//	printk("show_power_source ret = %d\n",power_source);
	switch (power_source) {
                        ...
                        char *buf)
{
	...
	int battery_voltage = 0;
	enum charging_currents value;
	int level_0 = 0, level_25 = 0, level_50 = 0, level_75 = 0,level_5 = 0;

	battery_voltage = priv->smooth_battery_mv;
	//printk("show_battery_percentage battery_voltage = %d\n",battery_voltage);
	level_0 = BATTERY_0;
	level_5 = BATTERY_5;
	level_25 = BATTERY_25;
	level_50 = BATTERY_50;
	...
	if ((battery_voltage >= 0) && (battery_voltage < level_0))
		ret = sprintf(buf, "0 \n");
	else if ((battery_voltage >= level_0) && (battery_voltage < level_5))
		ret = sprintf(buf, "5 \n");
	else if ((battery_voltage >= level_0) && (battery_voltage < level_25))
		ret = sprintf(buf, "25 \n");
	else if ((battery_voltage >= level_25) && (battery_voltage < level_50))
		...
		ret = sprintf(buf, "100 \n");
	//printk("show_battery_percentage ret = %d\n",ret);		
		return ret;	
}
static DEVICE_ATTR(battery_percentage, S_IRUSR|S_IRGRP|S_IROTH, show_battery_percentage, NULL);
...
{
	struct lf2000_hwmon *priv = (struct lf2000_hwmon *)data;
	schedule_work(&priv->power_button_work);
}
...
#if defined(CONFIG_PLAT_NXP4330_BOGOTA) || defined(CONFIG_PLAT_NXP4330_SANTIAGO)
...
#else
//	printk("lf2000_power_button %d\n",hwmon_dev->power_button_count);
...
#ifdef CONFIG_BATTERY_NXE2000
		case NXE2000:
			ret = nxe2000_is_usb_powered() ? 0 : 1;
			//ret = 1;
			break;
#endif
...
static void lf2000_set_battery(struct work_struct *work)
{
	...
	if (pass == 2)
	{
		...
		new_supply_mv = lf2000_get_battery_mv(hwmon_dev);
		//printk("lf2000_set_battery get battery = %d\n",new_supply_mv);
		exp = ((new_supply_mv*SCALE+(ALPHA_DIV-1)*exp)/ALPHA_DIV);
		hwmon_dev->smooth_battery_mv = (exp/SCALE);
...
#ifdef CONFIG_BATTERY_NXE2000
		case NXE2000:
			nxe2000_set_power_button_callback(
				lf2000_start_power_button_monitor);
			break;
#endif
...
static int lf2000_power_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_BATTERY_NXE2000
	if (nxe2000_have_nxe2000()) {
		priv->chip = NXE2000;
		dev_info(&pdev->dev, "%s.%d use NXE2000 power control\n",
			__FUNCTION__, __LINE__);
	}
#endif
	...
	priv->power_button_tasks =
		create_workqueue("power button tasks");
