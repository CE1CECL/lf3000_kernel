...
static int nxe2000_read_vthermbat(struct device *dev, int *data)
{
	uint8_t data_l = 0, data_h = 0;
	int uv_per_unit = 0;
	int ret;

	ret = nxe2000_read(dev, NXE2000_REG_VTHMDATAH, &data_h);
	if (ret < 0) {
		dev_err(&nxe2000_i2c_client->dev, "Error in reading the control register\n");
	}

	ret = nxe2000_read(dev, NXE2000_REG_VTHMDATAL, &data_l);
	if (ret < 0) {
		dev_err(&nxe2000_i2c_client->dev, "Error in reading the control register\n");
	}
...
void nxe2000_power_off(void)
{
...
#if defined(CONFIG_BATTERY_NXE2000)
	if (g_ichg_type) {
		nxe2000_read_vthermbat(&nxe2000_i2c_client->dev, &read_data);
		if (read_data < 825000) {  // disable
			g_ichg_poweroff = 0x00;
			printk(KERN_ERR "## [\e[31m%s\e[0m():%d] THERMBAT btemp_old_vol:%duV, VUSBCHG_DIS\n", __func__, __LINE__, read_data);
		} else if (read_data < 1600000) { // 800mA, 1500mA
			g_ichg_poweroff = 0x11;
			printk(KERN_ERR "## [\e[31m%s\e[0m():%d] THERMBAT btemp_old_vol:%duV, VUSBCHG_EN\n", __func__, __LINE__, read_data);
		} else if (read_data < 1825000) { // 700mA, 700mA
	...
	nxe2000_update(&nxe2000_i2c_client->dev, NXE2000_REG_CHGISET, g_ichg_poweroff, 0x3F);
	if (g_ichg_poweroff > 0) {
		nxe2000_set_bits(&nxe2000_i2c_client->dev, NXE2000_REG_CHGCTL1, 0x3);
	} else {
		nxe2000_clr_bits(&nxe2000_i2c_client->dev, NXE2000_REG_CHGCTL1, 0x3);
	}

	nxe2000_read(&nxe2000_i2c_client->dev, NXE2000_REG_CHGCTL1, &reg_val);
	printk(KERN_ERR"## NXE2000_REG_CHGCTL1  :0x%02x, val:0x%02x \n", NXE2000_REG_CHGCTL1, reg_val);
	nxe2000_read(&nxe2000_i2c_client->dev, NXE2000_REG_REGISET1, &reg_val);
	printk(KERN_ERR"## NXE2000_REG_REGISET1 :0x%02x, val:0x%02x \n", NXE2000_REG_REGISET1, reg_val);
	nxe2000_read(&nxe2000_i2c_client->dev, NXE2000_REG_REGISET2, &reg_val);
	printk(KERN_ERR"## NXE2000_REG_REGISET2 :0x%02x, val:0x%02x \n", NXE2000_REG_REGISET2, reg_val);
	nxe2000_read(&nxe2000_i2c_client->dev, NXE2000_REG_CHGISET, &reg_val);
	printk(KERN_ERR"## NXE2000_REG_CHGISET :0x%02x, val:0x%02x \n", NXE2000_REG_CHGISET, reg_val);
	nxe2000_read(&nxe2000_i2c_client->dev, NXE2000_REG_CHGSTATE, &reg_val);
	printk(KERN_ERR"## NXE2000_REG_CHGSTATE:0x%02x, val:0x%02x \n", NXE2000_REG_CHGSTATE, reg_val);
#endif

	/* Power OFF */
	printk(KERN_ERR"## NXE2000 Power Off!!!\n");
...
//	ret = pdata->init_port(client->irq);
	...
	if (client->irq) {
		...
		nxe2000->chip_irq       = IRQ_ALIVE_5;//gpio_to_irq(client->irq); 
