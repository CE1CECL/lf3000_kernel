...
static irqreturn_t nxe2000_irq_isr(int irq, void *data)
{
	...
	ret = nxe2000_read(nxe2000->dev, NXE2000_INTC_INTMON, &master_int);
	if (ret < 0) {
		dev_err(nxe2000->dev, "Error in reading reg 0x%02x "
			"error: %d\n", NXE2000_INTC_INTMON, ret);
		return IRQ_HANDLED;
	}
	PM_LOGOUT("## [%s():%d\t] master_int : 0x%x\n", __FUNCTION__, __LINE__, master_int);
	...
	for (i = 0; i < MAX_INTERRUPT_MASKS; ++i) {
		...
		if (ret < 0) {
			dev_err(nxe2000->dev, "Error in reading reg 0x%02x error: %d\n", irq_mon_add[i], ret);
			int_sts[i] = 0;
			continue;
		}
		PM_LOGOUT("## [%s():%d\t] i:%d, irq_mon_add[i]:0x%x, int_sts[i]:0x%x \n", __FUNCTION__, __LINE__, i, irq_mon_add[i], int_sts[i]);
		if (!int_sts[i])
			continue;

		if (main_int_type[i] & RTC_INT) {
			PM_LOGOUT("## [%s():%d\t] RTC_INT \n", __FUNCTION__, __LINE__);
		...
		{
			PM_LOGOUT("## [%s():%d\t] i:%d, irq_clr_add[i]0x%x, ~int_sts[i]:0x%x \n", __FUNCTION__, __LINE__, i, irq_clr_add[i], ~int_sts[i]);
		...
		if (main_int_type[i] & CHG_INT) {
			PM_LOGOUT("## [%s():%d\t] CHG_INT \n", __FUNCTION__, __LINE__);
			if (int_sts[i])
				ret = nxe2000_write(nxe2000->dev, irq_en_add[i], 0xff);
	...
	for (i = 0; i < NXE2000_NR_IRQS; ++i) {
		...
		if ((int_sts[data->mask_reg_index] & (1 << data->int_en_bit)) &&
			(nxe2000->group_irq_en[data->master_bit] &
					(1 << data->grp_index))) 
		{
			PM_LOGOUT("## [%s():%d\t] int_sts[%d]:0x%x, i:%d\n", __FUNCTION__, __LINE__, data->mask_reg_index, int_sts[data->mask_reg_index], i);
