...
static const u16 nxe2000_mask_reg[] = {
	...
	[NXE2000_IRQGRP_DCDC_INT]	= NXE2000_REG_TYPE_INTEN | NXE2000_REG_DCIRQ,
...
static void nxe2000_irq_sync_unlock(struct irq_data *data)
{
	...
	if (gpio_irq_type) {
		nxe2000_write_reg(i2c, NXE2000_REG_GPEDGE1, gpio_irq_type);
	}
	...
	for (i = 0; i < NXE2000_IRQ_GROUP_NR; i++) {
		...
		{
			nxe2000_write_reg(i2c, (u8)(nxe2000_mask_reg[i] & 0xFF),
...
//	DBGOUT("%s\n", __func__);
	...
	ret = nxe2000_read_reg(nxe2000->i2c, NXE2000_REG_INTMON, &irq_src);
	if (ret < 0) {
	...
	if (!irq_src)
	{
		nxe2000_read_reg(nxe2000->i2c, NXE2000_REG_CHGSTATE,
				&status_regs[NXE2000_IRQGRP_FG_INT]);
		status_regs[NXE2000_IRQGRP_CHG_INT1] = (1 << NXE2000_POS_CHGCTRL_MONI_RDSTATESHIFT);
		printk("chg state [%02xh - 0x%02x]\n", NXE2000_REG_CHGSTATE, status_regs[NXE2000_IRQGRP_FG_INT]);
	}
	...
	if (irq_src & NXE2000_IRQSRC_DCDC) {
		nxe2000_read_reg(nxe2000->i2c, NXE2000_REG_DCIRQ,
				&status_regs[NXE2000_IRQGRP_DCDC_INT]);
	}
	if (irq_src & NXE2000_IRQSRC_ADC) {
		/* ADC INT1 ~ INT3 */
		nxe2000_bulk_read(nxe2000->i2c, NXE2000_REG_IR_ADC1, 3,
	...
	if (irq_src & NXE2000_IRQSRC_GPIO) {
		...
		nxe2000_read_reg(nxe2000->i2c, NXE2000_REG_GPEDGE1,
				&gpio_edge_info);
		nxe2000_bulk_read(nxe2000->i2c, NXE2000_REG_EN_GPIR, 3,
				gpio_intr_info);
	...
	if (irq_src & NXE2000_IRQSRC_CHG) {
		nxe2000_bulk_read(nxe2000->i2c, NXE2000_REG_CHGCTL_MONI, 4,
	...
	for (i = 0; i < NXE2000_IRQ_GROUP_NR; i++)
	{
		...
		if (status_regs[i])
		{
			...
			{
				nxe2000_update_reg(nxe2000->i2c, (u8)(nxe2000_mask_reg[i] & 0xFF), ~status_regs[i], status_regs[i]);
			}
			else
			{
//				nxe2000_update_reg(nxe2000->i2c, (u8)(nxe2000_mask_reg[i] & 0xFF), status_regs[i], status_regs[i]);
				nxe2000_write_reg(nxe2000->i2c, (u8)(nxe2000_pend_reg[i] & 0xFF), status_regs[i]);
			}
		}
	}
...
int nxe2000_irq_init(struct nxe2000_dev *nxe2000)
{
	...
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_INTEN,		0x00);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_DCIREN,		0x00);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_EN_ADCIR1,	0x00);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_EN_ADCIR2,	0x00);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_EN_ADCIR3,	0x00);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_EN_GPIR,	0x00);
	...
	for (i = 0; i < NXE2000_IRQ_GROUP_NR; i++) {
		...
		nxe2000_write_reg(nxe2000->i2c, (u8)(nxe2000_mask_reg[i] & 0xFF), 0xff);
	}

	/* Pending clear */
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_DCIRQ,		0x00);

	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_IR_ADC1,	0x00);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_IR_ADC2,	0x00);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_IR_ADC3,	0x00);

	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_IR_GPR,		0x00);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_IR_GPF,		0x00);

	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_CHGCTL_IRR,		0x00);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_CHGSTAT_IRR1,	0x00);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_CHGSTAT_IRR2,	0x00);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_CHGERR_IRR,		0x00);
	...
	if (nxe2000->irq) {
		ret = request_threaded_irq(nxe2000->irq, NULL, nxe2000_irq_thread,
	...
	/* Interrup enable */
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_EN_ADCIR1,	0xff);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_EN_ADCIR2,	0xff);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_EN_ADCIR3,	0xff);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_EN_GPIR,	0xff);
//	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_DCIREN, 	0xff);
	nxe2000_write_reg(nxe2000->i2c, NXE2000_REG_INTEN,		0xdb);

	if (!nxe2000->ono)
		return 0;

	ret = request_threaded_irq(nxe2000->ono, NULL, nxe2000_irq_thread,
