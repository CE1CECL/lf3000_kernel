...
struct nxe2000_battery_info {
	...
	struct delayed_work	btemp_adc_work;
	...
	int				ch_ichg;
	int				ch_ilim_adp;
	int				ch_ilim_adp_data;
	int				ch_ilim_usb;
	int				ch_ilim_usb_data;
	int				ch_icchg;
	int				ch_ichg_slp;
	int 			ch_ichg_disable;
	int				fg_target_vsys;
...
static int charger_irq = 0;

int g_soc = 0;
int g_fg_on_mode = 0;
int	g_ichg_poweroff = 0;
int	g_ichg_type = 0;
...
static void	(*power_button_callback)(void) = NULL;
static int  have_nxe2000=0;
static int  power_key_press = 0;
static int is_usb_powered = 0;
...
static int measure_vthermbat_ADC(struct nxe2000_battery_info *info, int *data);
	...
	*is_charging = true;	/* currrent state initialize -> charging */
	...
	if (cc_stop_flag == 1)
	{
		...
		err = nxe2000_set_bits(info->dev->parent,
						NXE2000_REG_CHGSTAT_IRFMASK1, 0x01);
		if (err < 0)
			goto out;
...
end_flow:
...
#ifdef ENABLE_DEBUG
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] val:0x%x, power_type:0x%x \n", __func__, __LINE__, val, info->input_power_type);

	nxe2000_read(info->dev->parent, NXE2000_REG_ADCCNT1, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_ADCCNT1 :0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_ADCCNT1, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_EN_ADCIR1, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_EN_ADCIR1 :0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_EN_ADCIR1, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_EN_ADCIR2, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_EN_ADCIR2 :0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_EN_ADCIR2, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGSTATE, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGSTATE :0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGSTATE, val);
	...
	nxe2000_read(info->dev->parent, NXE2000_REG_REGISET1, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_REGISET1:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_REGISET1, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_REGISET2, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_REGISET2:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_REGISET2, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGISET, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGISET:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGISET, val);
	...
	nxe2000_read(info->dev->parent, NXE2000_REG_INTEN, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_INTEN:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_INTEN, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_INTMON, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_INTMON:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_INTMON, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGCTL_IRFMASK, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGCTL_IRFMASK:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGCTL_IRFMASK, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGSTAT_IRFMASK1, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGSTAT_IRFMASK1:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGSTAT_IRFMASK1, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGSTAT_IRFMASK2, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGSTAT_IRFMASK2:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGSTAT_IRFMASK2, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGERR_IRFMASK, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGERR_IRFMASK:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGERR_IRFMASK, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGCTL_IRR, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGCTL_IRR:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGCTL_IRR, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGSTAT_IRR1, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGSTAT_IRR1:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGSTAT_IRR1, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGSTAT_IRR2, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGSTAT_IRR2:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGSTAT_IRR2, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGERR_IRR, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGERR_IRR:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGERR_IRR, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGCTL_MONI, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGCTL_MONI:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGCTL_MONI, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGSTAT_MONI1, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGSTAT_MONI1:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGSTAT_MONI1, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGSTAT_MONI2, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGSTAT_MONI2:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGSTAT_MONI2, val);

	nxe2000_read(info->dev->parent, NXE2000_REG_CHGERR_MONI, &val);
	PM_LOGOUT( "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGERR_MONI:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGERR_MONI, val);
...
static int nxe2000_init_fgsoca(struct nxe2000_battery_info *info)
{
	...
	if (info->first_pwon) {
		...
		err = nxe2000_read(info->dev->parent, NXE2000_REG_CHGISET, &val);
		if (err < 0)
			dev_err(info->dev,
			"Error in read NXE2000_REG_CHGISET%d\n", err);

		err = nxe2000_write(info->dev->parent, NXE2000_REG_CHGISET, 0);
		if (err < 0)
			dev_err(info->dev,
			"Error in writing NXE2000_REG_CHGISET%d\n", err);
		/* msleep(1000); */
		...
		err = nxe2000_write(info->dev->parent, NXE2000_REG_CHGISET, val);
		if (err < 0)
			dev_err(info->dev,
			"Error in writing NXE2000_REG_CHGISET%d\n", err);
	}
...
#if (CFG_SW_UBC_ENABLE == 1)
#define TYPE_USB_SIGNATURE			0xDEAD0001 /* USB  */
#define TYPE_TA_SIGNATURE			0xDEAD0002 /* TA  */
#define TYPE_NONE_SIGNATURE		0xDEAD0003 /* NONE  */
...
static void nxe2000_sw_ubc_work(struct work_struct *work)
{
...
#if 0
	if (val != info->ch_ilim_usb)
		info->flag_set_ilimit = false;

	if (pmic_vbus && !info->flag_set_ilimit && !info->is_sdp_type) {
		ret = nxe2000_write(info->dev->parent, NXE2000_REG_REGISET2, info->ch_ilim_usb);
		if (ret < 0) {
	...
	if (info->is_sdp_type) {
		writel(0x0, SCR_USER_SIG7_RESET); /* clear */
		ret = nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x3);
		if (ret < 0)
			ret = nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x3);

		temp = (val & 0xE0) | info->ch_ilim_usb_data;
		temp1 = info->ch_ilim_adp_data;
		g_ichg_type = 1;
        printk(KERN_ERR "## [\e[31m%s\e[0m():%d] Clr ChgCtl1", __func__, __LINE__);
	} else {
		ret = nxe2000_set_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x3);
		if (ret < 0)
			ret = nxe2000_set_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x3);

		temp = (val & 0xE0) | info->ch_ilim_usb;
		temp1 = info->ch_ilim_adp;
		g_ichg_type = 0;
        printk(KERN_ERR "## [\e[31m%s\e[0m():%d] Set ChgCtl1", __func__, __LINE__);
	}

	if (info->ch_ichg_disable) {
		ret = nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x3);
		if (ret < 0)
			ret = nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x3);
        printk(KERN_ERR "## [\e[31m%s\e[0m():%d] Clr ChgCtl1", __func__, __LINE__);
	...
	printk(KERN_ERR "## [\e[31m%s\e[0m():%d] is_sdp_type:0x%x, pmic_vbus:0x%x, flag_set_ilimit:0x%x, ch_ichg_disable:0x%x, temp:0x%x, temp1:0x%x\n",
						__func__, __LINE__, info->is_sdp_type, pmic_vbus, info->flag_set_ilimit, info->ch_ichg_disable, temp, temp1);

	if (pmic_vbus && !info->flag_set_ilimit) {
		if (first_init == 0) {//&& (info->pmic_vbuschk_count == 0)) {
			first_init = 1;
			dwData=readl(SCR_USER_SIG7_READ);
			writel(0x0, SCR_USER_SIG7_RESET); /* clear */
			if (dwData == TYPE_USB_SIGNATURE) {
				ret = nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x3);
				if (ret < 0)
					ret = nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x3);

				info->pmic_vbuschk_count = 0;
				temp = (val & 0xE0) | info->ch_ilim_usb_data;
				temp1 = info->ch_ilim_adp_data;
				info->is_sdp_type = 0x1;
				g_ichg_type = 1;
				printk(KERN_ERR "## [\e[31m%s\e[0m():%d] is_sdp_type:0x%x, pmic_vbus:0x%x, flag_set_ilimit:0x%x, temp:0x%x, temp1:0x%x, dwData:0x%x\n",
						__func__, __LINE__, info->is_sdp_type, pmic_vbus, info->flag_set_ilimit, temp, temp1, dwData);
		        printk(KERN_ERR "## [\e[31m%s\e[0m():%d] Clr ChgCtl1", __func__, __LINE__);
			}
		}
		...
		ret = nxe2000_write(info->dev->parent, NXE2000_REG_REGISET1, temp1);
		if (ret < 0)
			ret = nxe2000_write(info->dev->parent, NXE2000_REG_REGISET1, temp1);
		if (ret < 0) {
			dev_err(info->dev, "Can't write NXE2000_REG_REGISET1 register. : %d\n", ret);
		}
	...
	nxe2000_read(info->dev->parent, NXE2000_REG_CHGCTL1, &val);
	printk(KERN_ERR "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGCTL1 :0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGCTL1, val);
	nxe2000_read(info->dev->parent, NXE2000_REG_REGISET1, &val);
	printk(KERN_ERR "## [\e[31m%s\e[0m():%d] NXE2000_REG_REGISET1:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_REGISET1, val);
	nxe2000_read(info->dev->parent, NXE2000_REG_REGISET2, &val);
	printk(KERN_ERR "## [\e[31m%s\e[0m():%d] NXE2000_REG_REGISET2:0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_REGISET2, val);
	nxe2000_read(info->dev->parent, NXE2000_REG_CHGISET, &val);
	printk(KERN_ERR "## [\e[31m%s\e[0m():%d] NXE2000_REG_CHGISET :0x%02x, val:0x%02x \n", __func__, __LINE__, NXE2000_REG_CHGISET, val);
#endif
...
*/
{
	...
	*is_jeita_updated = false;
	...
	/* Read CHGISET */
	err = nxe2000_read(info->dev->parent, NXE2000_REG_CHGISET, &chgiset_org);
	if (err < 0) {
	...
	if (temp <= 0 || 55 <= temp) {
        ...
        printk(KERN_ERR "## [\e[31m%s\e[0m():%d] Clr ChgCtl1",
						__func__, __LINE__);
		info->soca->jt_limit = 0;
		*is_jeita_updated = true;
		...
		/* set ICHG */
		err = nxe2000_write(info->dev->parent, NXE2000_REG_CHGISET, set_ichg_l);
		if (err < 0) {
			dev_err(info->dev, "Error in writing the battery setting register\n");
			goto out;
		}
        ...
        printk(KERN_ERR "## [\e[31m%s\e[0m():%d] Set ChgCtl1",
						__func__, __LINE__);
		...
		/* set ICHG */
		err = nxe2000_write(info->dev->parent, NXE2000_REG_CHGISET, set_ichg_h);
		if (err < 0) {
			dev_err(info->dev, "Error in writing the battery setting register\n");
			goto out;
		}
...
static int nxe2000_init_charger(struct nxe2000_battery_info *info)
{
	...
	if ((info->ch_ilim_adp != 0xFF) || (info->ch_ilim_adp <= 0x1D)) {
		...
		err = nxe2000_write(info->dev->parent, NXE2000_REG_REGISET1, val);
		if (err < 0) {
			dev_err(info->dev,
				"Error in writing NXE2000_REG_REGISET1 %d\n", err);
			goto free_device;
		}
	}
#endif
	...
	err = nxe2000_read(info->dev->parent, NXE2000_REG_REGISET2, &val);
	if (err < 0) {
		dev_err(info->dev,
	 		"Error in read NXE2000_REG_REGISET2 %d\n", err);
		goto free_device;
	}

#if 0
	if ((info->ch_ilim_usb_data != 0xFF) || (info->ch_ilim_usb_data <= 0x1D)) {
		val2 = info->ch_ilim_usb_data;
	} else {/* Keep OTP value */
		val2 = (val & 0x1F);
	}
	...
	err = nxe2000_write(info->dev->parent, NXE2000_REG_REGISET2,val);
	if (err < 0) {
		dev_err(info->dev,
			"Error in writing NXE2000_REG_REGISET2 %d\n", err);
		goto free_device;
	}
#endif
	...
	//debug messeage
	err = nxe2000_read(info->dev->parent, NXE2000_REG_CHGISET,&val);
	PM_LOGOUT("PMU : %s : after NXE2000_REG_CHGISET (0x%x) is 0x%x info->ch_ichg is 0x%x info->ch_icchg is 0x%x\n",__func__,NXE2000_REG_CHGISET,val,info->ch_ichg,info->ch_icchg);
	...
	err = nxe2000_write(info->dev->parent, NXE2000_REG_CHGSTAT_IRFMASK1, 0x0c);
	if (err < 0) {
	...
	// Charge Status Interrupt Detect Mode2 Register (Address CDh)
	//nxe2000_write(info->dev->parent, NXE2000_CHG_STAT_DETMOD2, 0x00);
...
static void charger_irq_work(struct work_struct *work)
{
	...
	ret = nxe2000_write(info->dev->parent, NXE2000_REG_CHGSTAT_IRFMASK1, 0x0c);
	if (ret < 0)
		dev_err(info->dev, "%s(): Error in enable charger mask INT %d\n", __func__, ret);
	...
	switch (val)
	{
		...
		case SUPPLY_STATE_ADP: // plug in ADP
			printk(KERN_ERR "## [%s():%s:%d\t] SUPPLY_STATE_ADP \n", __FUNCTION__, strrchr(__FILE__, '/')+1, __LINE__);
			if (info->input_power_type == INPUT_POWER_TYPE_UBC)
			{
			...
			else if (info->input_power_type == INPUT_POWER_TYPE_ADP_UBC)
			{
				...
				//if ((info->ch_ilim_adp != 0xFF) || (info->ch_ilim_adp <= 0x1D))
				//	nxe2000_write(info->dev->parent, NXE2000_REG_REGISET1, info->ch_ilim_adp);
		...
		case SUPPLY_STATE_USB:// plug in USB
			printk(KERN_ERR "## [%s():%s:%d\t] SUPPLY_STATE_USB \n", __FUNCTION__, strrchr(__FILE__, '/')+1, __LINE__);
...
static irqreturn_t power_on_isr(int irq, void *battery_info)
{
	struct nxe2000_battery_info *info = battery_info;

//	printk("power_on_key irq\n");

	if (power_button_callback != NULL) {
		power_button_callback();
		power_key_press++;
	} else {
		printk(KERN_ERR "%s power_button cb is NULL \n", __func__);
    }
	return IRQ_HANDLED;
}

void nxe2000_set_power_button_callback(void (*callback)(void))
{
        power_button_callback = callback;
}
EXPORT_SYMBOL(nxe2000_set_power_button_callback);

int nxe2000_get_power_button(void)
{
//	printk("nxe2000_get_power_button %d\n",power_key_press);
	return (power_key_press%2);
}
EXPORT_SYMBOL(nxe2000_get_power_button);

int nxe2000_have_nxe2000(void)
{
		return have_nxe2000;
}
EXPORT_SYMBOL(nxe2000_have_nxe2000);


int get_cur_voltage=0;

int nxe2000_get_current_voltage(void)
{
		return get_cur_voltage;
}
EXPORT_SYMBOL(nxe2000_get_current_voltage);


int nxe2000_is_usb_powered(void)
{

	return is_usb_powered;
}
EXPORT_SYMBOL(nxe2000_is_usb_powered);

static int power_status = 0;
int nxe2000_power_status(void)
{
...
EXPORT_SYMBOL(nxe2000_power_status);
...
static void nxe2000_btemp_adc_work(struct work_struct *work)
{
	struct nxe2000_battery_info *info = container_of(work, struct nxe2000_battery_info, btemp_adc_work.work);

	measure_vthermbat_ADC(info, &info->btemp_old_vol);

	return;
}
static irqreturn_t charger_btempjta1_isr(int irq, void *battery_info)
{
	struct nxe2000_battery_info *info = battery_info;
	int value;
	uint8_t set_val = 0;

	nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGSTAT_IRFMASK1, 0x10);
	measure_vthermbat_ADC(info, &value);

	info->ch_ichg = 0x06;
	info->ch_ichg_slp = 0x06;
	g_ichg_poweroff = 0x06;

	if (value > info->btemp_old_vol) {
		printk(KERN_ERR "## [\e[31m%s\e[0m():%d] THERMBAT btemp_old_vol:%duV, Vol:%duV, VUSBCHG_DIS\n", __func__, __LINE__, info->btemp_old_vol, value);
	} else {
		printk(KERN_ERR "## [\e[31m%s\e[0m():%d] THERMBAT btemp_old_vol:%duV, Vol:%duV, VUSBCHG_EN\n", __func__, __LINE__, info->btemp_old_vol, value);
	}

	set_val = (info->ch_icchg << NXE2000_POS_CHGISET_ICCHG) + info->ch_ichg;
	nxe2000_write(info->dev->parent, NXE2000_REG_CHGISET, set_val);

	queue_delayed_work(info->monitor_wqueue, &info->btemp_adc_work, msecs_to_jiffies(500));

	return IRQ_HANDLED;
}

static irqreturn_t charger_btempjta2_isr(int irq, void *battery_info)
{
	struct nxe2000_battery_info *info = battery_info;
	int value;
	uint8_t set_val = 0;

	nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGSTAT_IRFMASK1, 0x20);
	measure_vthermbat_ADC(info, &value);
	...
	if (value > info->btemp_old_vol)
	{
		...
		info->ch_ichg = 0x07;
		info->ch_ichg_slp = 0x11;
		g_ichg_poweroff = 0x11;
	}

	printk(KERN_ERR "## [\e[31m%s\e[0m():%d] THERMBAT btemp_old_vol:%duV, Vol:%duV, ichg_run:%dmA, ichg_sleep:%dmA\n", __func__, __LINE__, 
						info->btemp_old_vol, value, (info->ch_ichg+1)*100, (info->ch_ichg_slp+1)*100);
...
static irqreturn_t charger_btempjta3_isr(int irq, void *battery_info)
{
	struct nxe2000_battery_info *info = battery_info;
	int value;
	uint8_t set_val = 0;

	nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGSTAT_IRFMASK1, 0x40);
	measure_vthermbat_ADC(info, &value);
	...
	if (value > info->btemp_old_vol)
	{
		...
		info->ch_ichg_slp = 0x11;
		g_ichg_poweroff = 0x11;
		info->ch_ichg_disable = false;
        printk(KERN_ERR "## [\e[31m%s\e[0m():%d] ch_ichg_disable=%s\n",
                        __func__, __LINE__, info->ch_ichg_disable?"true":"false");
		if (info->is_sdp_type)
			nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x03);
		else
			nxe2000_set_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x03);
		printk(KERN_ERR "## [\e[31m%s\e[0m():%d] THERMBAT btemp_old_vol:%duV, Vol:%duV, ichg_run:%dmA, ichg_sleep:%dmA, VUSBCHG_EN\n", __func__, __LINE__, 
						info->btemp_old_vol, value, (info->ch_ichg+1)*100, (info->ch_ichg_slp+1)*100);
	} else {
		info->ch_ichg = 0x00;
		info->ch_ichg_slp = 0x00;
		g_ichg_poweroff = 0x00;
		info->ch_ichg_disable = true;
        printk(KERN_ERR "## [\e[31m%s\e[0m():%d] ch_ichg_disable=%s\n",
						__func__, __LINE__, info->ch_ichg_disable?"true":"false");
		nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x03);
		printk(KERN_ERR "## [\e[31m%s\e[0m():%d] THERMBAT btemp_old_vol:%duV, Vol:%duV, ichg_run:%dmA, ichg_sleep:%dmA, VUSBCHG_DIS\n", __func__, __LINE__, 
						info->btemp_old_vol, value, (info->ch_ichg+1)*100, (info->ch_ichg_slp+1)*100);
	}
...
static irqreturn_t charger_btempjta4_isr(int irq, void *battery_info)
{
	struct nxe2000_battery_info *info = battery_info;
	int value;
	uint8_t set_val = 0;

	nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGSTAT_IRFMASK1, 0x80);
	measure_vthermbat_ADC(info, &value);
...
static int measure_vthermbat_ADC(struct nxe2000_battery_info *info, int *data)
{
	...
	ret = nxe2000_read(info->dev->parent, NXE2000_REG_VTHMDATAH, &data_h);
	if (ret < 0) {
		dev_err(info->dev, "Error in reading the control register\n");
	}

	ret = nxe2000_read(info->dev->parent, NXE2000_REG_VTHMDATAL, &data_l);
	if (ret < 0) {
		dev_err(info->dev, "Error in reading the control register\n");
	}
...
#if 0
	/* conversion unit 1 Unit is 1.83mv (7500/4095 mv) */
	*data = *data * 1000 * 5 / 2 / 4095;
	/* return unit should be 1uV */
	*data = *data * 1000;
#else
	/* conversion unit 1 Unit is 1831uv (7500/4095 mv) */
	uv_per_unit = 2500000 / 4095;
	*data = *data * uv_per_unit;
#endif
...
//	printk("nxe2000_batt_get_prop property is %d\n",psp);
	switch (psp) {
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		DBGOUT(info->dev, "Power Supply Status is %d\n",
							info->status);
		power_status = info->status;
		//printk("nxe2000 Power Supply Status is %d\n",info->status);
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		get_cur_voltage = info->cur_voltage;
		printk("nxe2000 get battery voltage is %d mV\n",info->cur_voltage);
		break;
...
static struct power_supply	powerac = {
		.name = "acpwr",
...
static struct power_supply	powerusb = {
		.name = "usbpwr",
...
static __devinit int nxe2000_battery_probe(struct platform_device *pdev)
{
	...
	g_ichg_poweroff = pdata->type[type_n].ch_ichg_slp;;

	info->ch_ilim_adp	= pdata->type[type_n].ch_ilim_adp;
	info->ch_ilim_usb	= 0xE0 | pdata->type[type_n].ch_ilim_usb;
	info->ch_ilim_adp_data	= pdata->type[type_n].ch_ilim_adp_data;
	info->ch_ilim_usb_data	= pdata->type[type_n].ch_ilim_usb_data;
	...
	INIT_DELAYED_WORK_DEFERRABLE(&info->btemp_adc_work, nxe2000_btemp_adc_work);
	...
	ret = request_threaded_irq(charger_irq + NXE2000_IRQ_POWER_ON,
					NULL, power_on_isr, IRQF_ONESHOT, "nxe2000_power_on", info);
	if (ret < 0) {
		dev_err(&pdev->dev,
			"Can't get PWR_ON IRQ for power down: %d\n", ret);
		goto out;
	}
	
	ret = request_threaded_irq(charger_irq + NXE2000_IRQ_FBTEMPJTA1INT,
					NULL, charger_btempjta1_isr, IRQF_ONESHOT, "nxe2000_btempjta1_int", info);
	if (ret < 0) {
		dev_err(&pdev->dev,
			"Can't get BTEMPJTA1 IRQ for chrager: %d\n", ret);
		goto out;
	}

	ret = request_threaded_irq(charger_irq + NXE2000_IRQ_FBTEMPJTA2INT,
					NULL, charger_btempjta2_isr, IRQF_ONESHOT, "nxe2000_btempjta2_int", info);
	if (ret < 0) {
		dev_err(&pdev->dev,
			"Can't get BTEMPJTA2 IRQ for chrager: %d\n", ret);
		goto out;
	}

	ret = request_threaded_irq(charger_irq + NXE2000_IRQ_FBTEMPJTA3INT,
					NULL, charger_btempjta3_isr, IRQF_ONESHOT, "nxe2000_btempjta3_int", info);
	if (ret < 0) {
		dev_err(&pdev->dev,
			"Can't get BTEMPJTA3 IRQ for chrager: %d\n", ret);
		goto out;
	}

	ret = request_threaded_irq(charger_irq + NXE2000_IRQ_FBTEMPJTA4INT,
					NULL, charger_btempjta4_isr, IRQF_ONESHOT, "nxe2000_btempjta4_int", info);
	if (ret < 0) {
		dev_err(&pdev->dev,
			"Can't get BTEMPJTA4 IRQ for chrager: %d\n", ret);
		goto out;
	}
...
//	nxe2000_power_suspend_status	= 0;
	...
	info->flag_set_ilimit = false;
	//queue_delayed_work(info->monitor_wqueue, &info->sw_ubc_work, msecs_to_jiffies(1000));
	queue_delayed_work(info->monitor_wqueue, &info->sw_ubc_work, msecs_to_jiffies(10000));
#else
	...
	nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGSTAT_IRFMASK1, 0xF0);
...
#if (CFG_SW_UBC_ENABLE == 1)
	{
		...
		if(dwData == TYPE_USB_SIGNATURE) {
			g_ichg_type = 1;
			printk(KERN_ERR "## [%s():%s:%d\t] TYPE_USB_SIGNATURE \n", __FUNCTION__, strrchr(__FILE__, '/')+1, __LINE__);
		} else if (dwData == TYPE_TA_SIGNATURE) {
			printk(KERN_ERR "## [%s():%s:%d\t] TYPE_TA_SIGNATURE \n", __FUNCTION__, strrchr(__FILE__, '/')+1, __LINE__);
		} else {
			printk(KERN_ERR "## [%s():%s:%d\t] TYPE_NONE_SIGNATURE \n", __FUNCTION__, strrchr(__FILE__, '/')+1, __LINE__);
		}

		if ((info->btemp_old_vol < 825000) && (dwData == TYPE_TA_SIGNATURE)) {  // disable
			info->ch_ichg = 0x00;
			info->ch_ichg_slp = 0x00;
			g_ichg_poweroff = 0x00;
			nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x03);
			set_val = (info->ch_icchg << NXE2000_POS_CHGISET_ICCHG) + info->ch_ichg;
			nxe2000_write(info->dev->parent, NXE2000_REG_CHGISET, set_val);
			printk(KERN_ERR "## [\e[31m%s\e[0m():%d] THERMBAT btemp_old_vol:%duV, VUSBCHG_DIS, %d\n", __func__, __LINE__, info->btemp_old_vol, info->ch_ichg_disable);
		} else if ((info->btemp_old_vol < 1600000) && (dwData == TYPE_TA_SIGNATURE)) { // 800mA, 1500mA
			info->ch_ichg = 0x07;
			info->ch_ichg_slp = 0x11;
			g_ichg_poweroff = 0x11;
			nxe2000_set_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x03);

			printk(KERN_ERR "## [\e[31m%s\e[0m():%d] THERMBAT btemp_old_vol:%duV, ichg_run:%dmA, ichg_sleep:%dmA, VUSBCHG_EN, %d\n", __func__, __LINE__, 
							info->btemp_old_vol, (info->ch_ichg+1)*100, (info->ch_ichg_slp+1)*100, info->ch_ichg_disable);
		} else if ((info->btemp_old_vol < 1825000) && (dwData == TYPE_TA_SIGNATURE)) { // 700mA, 700mA 
			info->ch_ichg = 0x06;
			info->ch_ichg_slp = 0x06;
			g_ichg_poweroff = 0x06;
			set_val = (info->ch_icchg << NXE2000_POS_CHGISET_ICCHG) + info->ch_ichg;
			...
			nxe2000_clr_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x03);
			if (info->btemp_old_vol < 825000) {  // disable
				info->ch_ichg_disable = true;
				info->ch_ichg = 0x00;
				info->ch_ichg_slp = 0x00;
				g_ichg_poweroff = 0x00;
			} else if (info->btemp_old_vol < 1600000) { // 800mA, 1500mA
				info->ch_ichg_disable = false;
				info->ch_ichg = 0x07;
				info->ch_ichg_slp = 0x11;
				g_ichg_poweroff = 0x11;
			} else if (info->btemp_old_vol < 1825000) { // 700mA, 700mA 
				info->ch_ichg_disable = false;
				info->ch_ichg = 0x06;
				info->ch_ichg_slp = 0x06;
				g_ichg_poweroff = 0x06;
...
static int __devexit nxe2000_battery_remove(struct platform_device *pdev)
{
	...
	if((0 < info->ch_ichg_slp) && (info->ch_ichg_slp <= 0x11))
	{
		set_val = (info->ch_icchg << NXE2000_POS_CHGISET_ICCHG) + info->ch_ichg_slp;
		nxe2000_write(info->dev->parent, NXE2000_REG_CHGISET, set_val);
		nxe2000_set_bits(info->dev->parent, NXE2000_REG_CHGCTL1, 0x3);
	}
	...
	cancel_delayed_work(&info->btemp_adc_work);
...
//	nxe2000_power_suspend_status    = 1;
	...
	flush_delayed_work(&info->btemp_adc_work);
...
static int nxe2000_battery_resume(struct device *dev) {
	...
	if((0 < info->ch_ichg_slp) && (info->ch_ichg_slp <= 0x11))
	{
		...
		nxe2000_write(info->dev->parent, NXE2000_REG_CHGISET, val);
	}
