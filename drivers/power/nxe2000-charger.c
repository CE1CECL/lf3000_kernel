...
//#define NXE2000_FULL_CAP	(2250)
#define NXE2000_FULL_CAP	(2500)
...
static int nxe2000_set_chg_current(struct i2c_client *i2c, enum nxe2000_pwr_src supply, u32 uA)
{
	const struct current_map_desc *desc;
	int min_mA, max_mA;
	u8	value;
	...
	value = nxe2000_get_current_proper_val(desc, min_mA, max_mA);

	switch (supply)
	{
	case NXE2000_PWR_SRC_BAT:
		nxe2000_update_reg(i2c, NXE2000_REG_CHGISET, value, NXE2000_VAL_CHGISET_ICHG_MASK);
		break;
	case NXE2000_PWR_SRC_ADP:
		nxe2000_write_reg(i2c, NXE2000_REG_REGISET1, value);
		break;
	case NXE2000_PWR_SRC_USB:
		nxe2000_write_reg(i2c, NXE2000_REG_REGISET2, value);
		break;
	}
...
static int nxe2000_power_check_online(u8 chg_state, enum nxe2000_pwr_src supply,
				     union power_supply_propval *val)
{
	...
	{
	case NXE2000_PWR_SRC_BAT:
		if (chg_state != NXE2000_VAL_CHGSTATE_NO_BATT && chg_state != NXE2000_VAL_CHGSTATE_NO_BATT2)
			val->intval = 1;
		break;
	case NXE2000_PWR_SRC_ADP:
	case NXE2000_PWR_SRC_USB:
		value = (chg_state >> NXE2000_POS_CHGSTATE_USEADP) & (1 << supply);
		if (value)
			val->intval = 1;
		break;
	}
...
static void nxe2000_config_battery(struct nxe2000_dev *iodev)
{
	...
	nxe2000_write_reg(iodev->i2c, NXE2000_REG_CHGCTL_IRFMASK,	0x3F);
#else
	nxe2000_write_reg(iodev->i2c, NXE2000_REG_CHGCTL_IRFMASK,	0x00);
	nxe2000_write_reg(iodev->i2c, NXE2000_REG_CHGSTAT_IRFMASK1,	0xF0);
	nxe2000_write_reg(iodev->i2c, NXE2000_REG_CHGSTAT_IRFMASK2,	0xF0);
	nxe2000_write_reg(iodev->i2c, NXE2000_REG_CHGERR_IRFMASK,	0x00);
#endif
#endif
}
...
static int nxe2000_bat_check_type(u8 chg_state, int *type)
{
	switch (chg_state & NXE2000_POS_CHGSTATE_RDSTATE_MASK) {
	case NXE2000_VAL_CHGSTATE_TRICKLE_CHG:
...
static int nxe2000_power_read_voltage(struct i2c_client *i2c,
				     union power_supply_propval *val)
{
	u8  value[2];
	int	ret;
	...
	ret = nxe2000_bulk_read(i2c, NXE2000_REG_VOLTAGE_1, 2, value);
	if (ret)
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		ret = nxe2000_power_check_online(chg_state, NXE2000_PWR_SRC_ADP, val);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		ret = nxe2000_power_read_voltage(i2c, val);
		break;
	default:
		ret = -EINVAL;
		break;
					...
					union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		ret = nxe2000_power_check_online(chg_state, NXE2000_PWR_SRC_USB, val);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		ret = nxe2000_power_read_voltage(i2c, val);
		break;
				     ...
				     union power_supply_propval *val)
{
	...
	ret = nxe2000_bulk_read(i2c, NXE2000_REG_TT_EMPTY_H, 2, value);
	if (ret)
				     ...
				     union power_supply_propval *val)
{
	...
	ret = nxe2000_bulk_read(i2c, NXE2000_REG_TT_FULL_H, 2, value);
	if (ret)
				     ...
				     union power_supply_propval *val)
{
	...
	ret = nxe2000_bulk_read(i2c, NXE2000_REG_TEMP_1, 2, value);
	if (ret)
		goto exit_get_temp;
...
static int nxe2000_bat_get_prop(struct power_supply *psy,
		enum power_supply_property psp,
		union power_supply_propval *val)
{
	struct nxe2000_power	*nxe2000_power = dev_get_drvdata(psy->dev->parent);
...
#ifdef	NXE2000_USED_TIMER_THREAD
		ret = nxe2000_read_reg(i2c, NXE2000_REG_CHGSTATE,
				&chg_state);
		if (ret) {
			val->intval = 0;
		}
	...
	case POWER_SUPPLY_PROP_HEALTH:
		ret = nxe2000_read_reg(i2c, NXE2000_REG_CHGERR_MONI, &reg);
		if (ret)
			return ret;
	...
	case POWER_SUPPLY_PROP_PRESENT:	/* POWER_SUPPLY_STATUS_UNKNOWN, CHARGING, DISCHARGING, NOT_CHARGING, FULL */
		if (chg_state & (NXE2000_VAL_CHGSTATE_NO_BATT2 | NXE2000_VAL_CHGSTATE_NO_BATT))
			val->intval = 0;
		else
			val->intval = 1;
		break;
	case POWER_SUPPLY_PROP_ONLINE:	/* 0: OFF Line, 1: ON Line  */
		ret = nxe2000_power_check_online(chg_state, NXE2000_PWR_SRC_BAT, val);
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		ret = nxe2000_bat_check_type(chg_state, &val->intval);
		break;
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		ret = nxe2000_read_reg(i2c, NXE2000_REG_CHGISET, &reg);
		if (ret)
			val->intval = 0;
		else
			val->intval = ((reg & NXE2000_VAL_CHGISET_ICHG_MASK) + 1) * 100;	/* unit is mA */
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_NOW:
		...
		{
			ret = nxe2000_read_reg(i2c, NXE2000_REG_SOC, &reg);
			if (ret)
				val->intval = 0;
			else
...
static irqreturn_t nxe2000_pwr_src_irq(int irq, void *data)
{
	...
	nxe2000_read_reg(iodev->i2c, NXE2000_REG_FG_CTRL, &fg_ctrl);
...
static __devinit int nxe2000_power_probe(struct platform_device *pdev)
{
	...
	if (pdata->eoc_mA >= 10 && pdata->eoc_mA <= 45) {
		nxe2000_update_reg(i2c, NXE2000_REG_BATSET1,
	...
	nxe2000_set_chg_current(i2c, NXE2000_PWR_SRC_BAT, pdata->chg_current);
	nxe2000_set_chg_current(i2c, NXE2000_PWR_SRC_ADP, pdata->adp_ilim_current);
	nxe2000_set_chg_current(i2c, NXE2000_PWR_SRC_USB, pdata->usb_ilim_current);

	/* Feul Gauge Enable */
	nxe2000_update_reg(i2c, NXE2000_REG_FG_CTRL, 0x1 << NXE2000_POS_FG_CTRL_FG_EN, 0x1 << NXE2000_POS_FG_CTRL_FG_EN);

	/* Setup Charge Restart Level */
	switch (pdata->restart) {
	case 100:
		nxe2000_update_reg(i2c, NXE2000_REG_BATSET1, 0x1 << 3, 0x3 << 3);
		break;
	case 150:
		nxe2000_update_reg(i2c, NXE2000_REG_BATSET1, 0x0 << 3, 0x3 << 3);
		break;
	case 200:
		nxe2000_update_reg(i2c, NXE2000_REG_BATSET1, 0x2 << 3, 0x3 << 3);
		break;
	case -1:
		nxe2000_update_reg(i2c, NXE2000_REG_BATSET1, 0x3 << 3, 0x3 << 3);
		break;
	case 0:
		dev_dbg(power->dev,
	...
	case 5:
		nxe2000_update_reg(i2c, NXE2000_REG_TT_FULL_H, 0x0 << 4, 0x3 << 4);
		break;
	case 6:
		nxe2000_update_reg(i2c, NXE2000_REG_TT_FULL_H, 0x1 << 4, 0x3 << 4);
		break;
	case 7:
		nxe2000_update_reg(i2c, NXE2000_REG_TT_FULL_H, 0x2 << 4, 0x3 << 4);
		break;
	case -1:
		nxe2000_update_reg(i2c, NXE2000_REG_TT_FULL_H, 0x3 << 4, 0x3 << 4);
		break;
	case 0:
		dev_dbg(power->dev,
