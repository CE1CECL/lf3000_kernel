...
static int alc5623_jack_status_check(void)
{
	...
	if (!level) {
		...
		snd_soc_update_bits(codec, 0x04, 0x8080, 0x0);
		snd_soc_write(codec, 0x04, 0x0404);
#ifndef CONFIG_PLAT_NXP4330_SANTIAGO	/* temporary */
		...
		struct device_attribute *attr, char *buf)
{
	...
	nxp_alc5621 = i2c_get_clientdata(alc5623_card.rtd->codec->control_data);
...
static int alc5623_probe(struct platform_device *pdev)
{
	...
	printk("alc5623-dai: register card %s -> %s ret = %d rate=%d\n",
...
static int __init nxp_alc5623_audio_init(void)
{
	printk(KERN_DEBUG "%s: \n", __FUNCTION__);
	return platform_driver_register(&alc5623_driver);
}
module_init(nxp_alc5623_audio_init);

static void __exit nxp_alc5623_audio_exit(void)
{
	printk(KERN_DEBUG "%s: \n", __FUNCTION__);
	platform_driver_unregister(&alc5623_driver);
}
module_exit(nxp_alc5623_audio_exit);
