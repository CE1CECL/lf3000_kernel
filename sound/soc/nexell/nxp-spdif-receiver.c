...
static struct snd_soc_codec_driver soc_codec_spdif_rcv;

static struct snd_soc_dai_driver rcv_stub_dai = {
...
static int spdif_rcv_probe(struct platform_device *pdev)
{
	return snd_soc_register_codec(&pdev->dev, &soc_codec_spdif_rcv,
			&rcv_stub_dai, 1);
}

static int spdif_rcv_remove(struct platform_device *pdev)
{
...
static int spdifrx_probe(struct platform_device *pdev)
{
	...
	DBGOUT("spdif-rx-dai: register card %s -> %s\n",
 ...
 * SPDIF Receiver
 */
static struct platform_driver spdif_rcv_driver = {
	.probe		= spdif_rcv_probe,
	.remove		= spdif_rcv_remove,
	.driver		= {
...
module_platform_driver(spdif_rcv_driver);
