...
static void nxp_pcm_dma_complete(void *arg)
{
	...
	/*
		DBGOUT("snd pcm: %s complete offset = %8d (preiodbytes=%d)\n",
...
static int nxp_pcm_dma_slave_config(void *runtime_data, int stream)
{
	...
	DBGOUT("%s: %s %s, %s, addr=0x%x, bus=%d byte, burst=%d (%d)\n",
...
static int nxp_pcm_dma_prepare_and_submit(struct snd_pcm_substream *substream)
{
	...
	DBGOUT("buffer_bytes=%6d, period_bytes=%6d, periods=%2d, rate=%6d, period_time=%3d ms\n",
		snd_pcm_lib_buffer_bytes(substream), snd_pcm_lib_period_bytes(substream),
		substream->runtime->periods, substream->runtime->rate, period_time_ms);
	return 0;
}
...
static int nxp_pcm_open(struct snd_pcm_substream *substream)
{
	...
	prtd->dma_area = (unsigned int)runtime->dma_area;
					...
					struct snd_pcm_hw_params *params)
{
	 ...
	 */
	DBGOUT("%s: %s\n", __func__, STREAM_STR(substream->stream));
	DBGOUT("buffer_size =%6d, period_size =%6d, periods=%2d, rate=%6d\n",
		params_buffer_size(params),	params_period_size(params),
		params_periods(params), params_rate(params));
	DBGOUT("buffer_bytes=%6d, period_bytes=%6d, periods=%2d, period_time=%3d ms\n",
...
static int nxp_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	DBGOUT("%s: %s, dma_alloc_writecombine %d byte\n",
	...
	DBGOUT("%s: %s, dma_alloc_writecombine %d byte, vir = 0x%x, phy = 0x%x\n",
