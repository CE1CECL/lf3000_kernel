...
static char cpu_dai_name[16] = DEV_NAME_I2S;
static struct snd_soc_dai_link rt5631_dai_link = {
	.name 			= "ASOC-RT5631",
	.stream_name 	= "rt5631 HiFi",
	.cpu_dai_name 	= cpu_dai_name,			/* nxp_snd_i2s_driver name */
...
static int rt5631_probe(struct platform_device *pdev)
{
	...
	struct nxp_snd_jack_pin *hpin;
	struct snd_soc_jack_gpio *jack;
	unsigned int sample_rate = 0, pcm_format = 0;
	...
	if (plat) {
		if(plat->sample_rate)
			sample_rate = plat->sample_rate;

		if(plat->pcm_format)
			pcm_format = plat->pcm_format;

		/* set sound cpu name */
		if (plat->snd_device)
			sprintf(cpu_dai_name, "%s", DEV_NAME_I2S);
		else
			sprintf(cpu_dai_name, "%s.%d", DEV_NAME_I2S, plat->snd_dev_ch);

		/* jack detect */
		hpin = &plat->hp_jack;
		jack = &jack_gpio;
		if (hpin && hpin->support) {
	...
	/*
	 * Reset i2s support sample rates
	 */
	if (driver && sample_rate) {
	...
	/*
	 * Reset i2s support format
	 */
	if (driver && pcm_format) {
		driver->playback.formats = pcm_format;
		driver->capture.formats  = pcm_format;
	}
	DBGOUT("rt5623-dai: register card %s -> %s\n",
