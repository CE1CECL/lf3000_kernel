...
#define	SPDIF_MAX_BURST			16			// Byte
...
struct spdif_register {
	...
	unsigned int DAT;			// 0x0010	//   W	// SPDIF_IFOUT data buffer
	unsigned int CNT;			// 0x0014 	//   W	// repetition count register
...
static int calc_hdmi_master_clock(unsigned long mclk, int *pllsel, int *plldiv)
{
	...
	DBGOUT("%s: req=%ld, acq=%ld (pll[%d]=%ld, div=%2d)\n",
		__func__, request, rate_hz, pll_sel, clk_hz[pll_sel], pll_div);

	return 0;
}

static void inline spdif_reset(unsigned int base)
{
...
static int nxp_spdif_set_plat_param(struct nxp_spdif_snd_param *par, void *data)
{
	...
	DBGOUT("spdif-tx: %s, %s dma, addr 0x%x, bus %dbyte, burst %dbyte\n",
				 ...
				 struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		DBGOUT("spdiftx: change sample bits %s -> S16\n", PCM==PCM_16BIT?"S16":"S24");
		if (PCM != PCM_16BIT) {
	...
	case SNDRV_PCM_FORMAT_S24_LE:
		DBGOUT("spdiftx: change sample bits %s -> S24\n", PCM==PCM_16BIT?"S16":"S24");
		if (PCM != PCM_24BIT) {
			spdif->CON &= ~(0x3 << CON_PCM_BIT_POS);
