...
#define	EU_HP_Volume_Limit		(0x3A)
static char lfp100_locale[5];
#define US_CA_Region		(0)
#define NON_US_CA_Region	(1)
static unsigned char RegionSetting;
static signed long SpkVolValue=-1;

#define HP_Disconnected	 (0)
#define HP_Connected	 (1)
#define HP_StatusInvalid	 (2)
static unsigned char HPStatus=HP_StatusInvalid, PreHPStatus=HP_StatusInvalid;
...
static int lfp100_codec_probe(struct snd_soc_codec *codec)
{
	...
	//strncpy(lfp100_locale, "en-fr", sizeof(lfp100_locale));
	//strncpy(lfp100_locale, "en-us", sizeof(lfp100_locale));		
	if ((!strncasecmp(lfp100_locale, "en-us",5)) || (!strncasecmp(lfp100_locale, "en-ca",5)))
		RegionSetting = US_CA_Region;
	else
		RegionSetting = NON_US_CA_Region;
...
int lfp100_set_locale(const char * locale)
{
	strncpy(lfp100_locale, locale, sizeof(lfp100_locale));
	...
	else
	{
       ...
       //printk("????????        Ternece %s.%d locale=%s db\n",__FUNCTION__, __LINE__,lfp100_locale);
	return 1;
	
}
EXPORT_SYMBOL(lfp100_set_locale);

char * lfp100_get_locale(void)
{
	//printk(KERN_INFO "%s.%d \n",__FUNCTION__, __LINE__);


	return lfp100_locale;

}
EXPORT_SYMBOL(lfp100_get_locale);

int HP_Connect_Status_Update(int status2,unsigned char *DigVolReg)
{
	unsigned short val;
	...
	{
		if (PreHPStatus == HP_StatusInvalid)
		{
			PreHPStatus = HPStatus = (status2 & LFP100_STATUS2_HP) ? HP_Connected : HP_Disconnected;
			return -1;
		}
		else
		{
			HPStatus = (status2 & LFP100_STATUS2_HP) ? HP_Connected : HP_Disconnected;
		}
	}
	
	if (HPStatus == PreHPStatus)
	{
		return -1;
	}
	else
	{
		...
		if ((HPStatus == HP_Connected) && (RegionSetting == NON_US_CA_Region))
		{
			if ((val > EU_HP_Volume_Limit) || (val ==0))
			{
				val = EU_HP_Volume_Limit;
			}
		}

		*DigVolReg = (unsigned char) val;

		PreHPStatus = HPStatus;
