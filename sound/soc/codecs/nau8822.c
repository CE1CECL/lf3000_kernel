...
#ifdef DEBUGFS_ENABLED
#include <linux/debugfs.h>
#endif //DEBUGFS_ENABLED
...
static unsigned int nau8822_read_IIC(struct snd_soc_codec *codec, unsigned int addr)
{
	unsigned int value = 0;
	int ret = 0;
	struct nau8822_priv *nau8822 = snd_soc_codec_get_drvdata(codec);
	struct regmap *map = nau8822->regmap;
	ret = regmap_read(nau8822->regmap, addr, &value);
	if (ret == 0) {
...
static int nau8822_write_IIC(struct snd_soc_codec *codec, unsigned int addr, unsigned int value)
{
	u8 data[2];
	...
	if(addr==NAU8822_REG_LEFT_DAC_DIGITAL_VOLUME
			|| addr==NAU8822_REG_RIGHT_DAC_DIGITAL_VOLUME
			|| addr==NAU8822_REG_LEFT_ADC_DIGITAL_VOLUME
			|| addr==NAU8822_REG_RIGHT_ADC_DIGITAL_VOLUME
			|| addr==NAU8822_REG_LEFT_INP_PGA_CONTROL
			|| addr==NAU8822_REG_RIGHT_INP_PGA_CONTROL
			|| addr==NAU8822_REG_LHP_VOLUME
			|| addr==NAU8822_REG_RHP_VOLUME
			|| addr==NAU8822_REG_LSPKOUT_VOLUME
			|| addr==NAU8822_REG_RSPKOUT_VOLUME
			)  {
		//if(value == 1)
		value |= (1 << 8 );
	}
    //printk("\x1b[31m write: addr=0x%x value=0x%x \x1b[0m \n", addr, value);
	if (regmap_write(map, addr, value) != 0) {
		dev_err(codec->dev, "i2c write error\n");
		return -EIO;
	}

	if(addr==NAU8822_REG_LHP_VOLUME) {
		nau8822_write_IIC(codec, NAU8822_REG_RHP_VOLUME, value);
	}

	if(addr==NAU8822_REG_LSPKOUT_VOLUME) {
		nau8822_write_IIC(codec, NAU8822_REG_RSPKOUT_VOLUME, value);
	}
...
#ifdef DEBUGFS_ENABLED
static struct dentry * nau8822_debugfs_dentry = NULL;
static struct nau8822_priv* debug_nau8822 = NULL;
#endif //DEBUGFS_ENABLED
...
static const struct snd_kcontrol_new nau8822_snd_controls[] = {
	SOC_SINGLE_TLV("Speaker Playback Volume",
		NAU8822_REG_LSPKOUT_VOLUME, 0, 63, 0, spk_tlv),
	SOC_SINGLE("Speaker Playback Switch",
		NAU8822_REG_LSPKOUT_VOLUME, 6, 1, 1),
	SOC_SINGLE_TLV("Headphone Playback Volume",
		NAU8822_REG_LHP_VOLUME, 0, 63, 0, spk_tlv),
	SOC_SINGLE("Headphone Playback Switch",
		NAU8822_REG_LHP_VOLUME, 6, 1, 1),
};
#endif
...
{
	struct nau8822_priv *nau8822 = snd_soc_codec_get_drvdata(source->codec);
	unsigned int value;
				 ...
				 unsigned int freq, int dir)
{
	...
	struct nau8822_priv *nau8822 = snd_soc_codec_get_drvdata(codec);
		...
		nau8822->sysclk = freq;
		dev_err(nau8822->dev, "master sysclk %dHz, source %s\n",freq,
				...
				int rate)
{
	...
	case NAU8822_CLK_MCLK:
		...
		dev_err(nau8822->dev,"master clock prescaler %x for fs %d\n", div,
			rate);
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_ON:
		regmap_read(map, NAU8822_REG_POWER_MANAGEMENT_1, &value);
		if ((value & 0x10D != 0x10D))
			regmap_update_bits(map, NAU8822_REG_POWER_MANAGEMENT_1, 0x10D, 0x10D);
		regmap_read(map, NAU8822_REG_POWER_MANAGEMENT_3, &value);
		if ((value & 0x6F != 0x06F))
			regmap_update_bits(map, NAU8822_REG_POWER_MANAGEMENT_3, 0x06F, 0x06F);
	...
	case SND_SOC_BIAS_STANDBY:
		...
		regmap_write(map, NAU8822_REG_POWER_MANAGEMENT_1, 0x11D);
		regmap_write(map, NAU8822_REG_POWER_MANAGEMENT_2, 0x1BF);
		regmap_write(map, NAU8822_REG_POWER_MANAGEMENT_3, 0x6F);
		break;
...
static int nau8822_probe(struct snd_soc_codec *codec)
{
    ...
    dev_err(codec->dev, "%s:%d IN\n", __func__, __LINE__);
	/*
	 * Set the update bit in all registers, that have one. This way all
	 * writes to those registers will also cause the update bit to be
	 * written.
	 */
    regmap_write(nau8822->regmap, NAU8822_REG_CLOCKING, 0x0E0);//for test

    nau8822_set_bias_level(codec, SND_SOC_BIAS_STANDBY);

	regmap_write(nau8822->regmap, NAU8822_REG_INPUT_CONTROL, 0x03);
	regmap_write(nau8822->regmap, NAU8822_REG_LEFT_DAC_DIGITAL_VOLUME, 0xFF);
	regmap_write(nau8822->regmap, NAU8822_REG_RIGHT_DAC_DIGITAL_VOLUME, 0xFF);

	regmap_write(nau8822->regmap, NAU8822_REG_OUTPUT_CONTROL, 0x06);
	regmap_write(nau8822->regmap, NAU8822_REG_POWER_MANAGEMENT_1, 0x11D);
	regmap_write(nau8822->regmap, NAU8822_REG_POWER_MANAGEMENT_2, 0x1BF);
	regmap_write(nau8822->regmap, NAU8822_REG_POWER_MANAGEMENT_3, 0x6F);

	regmap_write(nau8822->regmap, NAU8822_REG_RIGHT_SPEAKER_CONTROL, 0x10);

	regmap_write(nau8822->regmap, NAU8822_REG_LEFT_INP_PGA_CONTROL, 0x3F);
	regmap_write(nau8822->regmap, NAU8822_REG_LEFT_ADC_BOOST_CONTROL, 0x000);

    for (i = 0; i < ARRAY_SIZE(update_reg); i++)
		regmap_update_bits(nau8822->regmap, update_reg[i], 0x100, 0x100);


	//snd_soc_add_codec_controls(codec, nau8822_snd_controls,
...
static struct snd_soc_codec_driver soc_codec_dev_nau8822 = {
	.write = 	nau8822_write_IIC,
	.read = 	nau8822_read_IIC,
...
static int nau8822_debugfs_show(struct seq_file *m, void *unused)
{
    int i;
    int num_reg_defaults = ARRAY_SIZE(nau8822_reg_defaults);
	unsigned int value = 0;
	int ret = 0;
    if (!debug_nau8822)
        return 0;
    ...
    for (i = 0; i < num_reg_defaults; i++) {
        ret = regmap_read(debug_nau8822->regmap, nau8822_reg_defaults[i].reg, &value);
        if (ret == 0)
            seq_printf(m, "0x%x\t\t 0x%x\n", nau8822_reg_defaults[i].reg, value);
    }
    seq_puts(m, "------------------\n");
    rcu_read_unlock();
    return 0;
...
static int nau8822_debugfs_open(struct inode *inode, struct file *file)
{
    return single_open(file, nau8822_debugfs_show, NULL);
}

static const struct file_operations nau8822_debugfs_fops = {
    .owner = THIS_MODULE,
    .open = nau8822_debugfs_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int __init nau8822_debugfs_init(void)
{
    nau8822_debugfs_dentry = debugfs_create_file("nau88c22",
            S_IRUGO, NULL, NULL, &nau8822_debugfs_fops);
    return 0;
}
postcore_initcall(nau8822_debugfs_init);
#endif //DEBUGFS_ENABLED
